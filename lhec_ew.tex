...
{\color{blue}Some very first text:}
The only $ep$ collider until today was HERA. First measurements of
electroweak effects have been undertaken in Refs.~\cite{todo}.
Three future projects are currently discussed, EIC, LHeC, and
FCC-$eh$.
The first, is already approved and will be built.
For these four $ep$ facilities, we compare single-differential NC and CC inclusive DIS cross sections
for polarized electron-proton scattering as a function of \Qsq\ in
figure~\cite{fig:dSigmaFacilities}.
{\color{blue}?? comment on EW effects for EIC here???}
\begin{figure}[htb]
  \centering
...
\end{figure}
The realisation of the FCC-$eh$ is in the far future, while LHeC may
potentially be realized during the lifetime of the LHC and may take
unique data during the 2030s.
Since the LHeC was recently been considered as a realistic option in
the EPPSU deliberation document, we therefore focus our studies on the
potential future LHeC kinematics.
Studies of electroweak effects for similar energies have been performed
in the past for LHeC (cite: CDR-2012) and earlier for the
LEP$\times$LHC proposal (cite earlier work).
However, the newly proposed energy-recovery linac, together with the
HL-LHC upgrade, will provide an extraordinarily increased integrated
luminosity over all previous studies.
This fact extends significantly the kinematic reach to higher
\Qsq\ and thus extends the sensitivity to electroweak effects, as will
be studied in the following.

We do not study direct $W$ or $Z$-production.
