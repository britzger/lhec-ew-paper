...
are discussed in the forthcoming thorough update of physics and conceptual 
...
DIS are presently studied. An electron-hadron collider is part 
of the possible Future Circular Collider (FCC), the 
FCC-$eh$~\cite{Abada:2019lih}, and will reach center-of-mass 
energies still higher than at the LHeC. At lower energies, the 
Electron Ion Collider 
(EIC)~\cite{Accardi:2012qut,Aschenauer:2017jsk,EICpreCDR}
will perform DIS measurements with higher luminosities than was 
...
important\,\footnote{However, it is 
...
We will put our focus on the measurement prospects 
...
simulated with different assumptions on the experimental 
uncertainties or the center-of-mass energy. Measurements in the 
regime of space-like momentum transfer, where the interaction is 
mediated by gauge boson exchange in the $t$-channel, is essentially 
...
precision of measurements at the LHeC could allow to also 
envisage an indirect determination of the top-quark mass through 
higher-order corrections (Sec.\,\ref{sec:mt}). These studies 
will allow one to perform tests of the SM by comparing them with 
...
the electroweak physics potential in the forthcoming publication 
...
was assumed.
Another data sample will be collected with a 
...
of 10\,fb$^{-1}$ is assumed for this sample\footnote{This value may actually be smaller
...
corrections are expected and the resulting experimental uncertainties
will therefore comprise these uncertainties.
An uncertainty due to the background 
from photoproduction events of $1.0~\%$ in the high-$y$ region 
is assumed. The statistical uncertainty of each data point is taken to be at least 
...
uncertainties, beam background related uncertaintes, possible
small stochastic uncertainties related to the calibration procedure,
or uncorrelated components of any of aboves systematic uncertainty.
Infact, the actual size of this uncorrelated uncertainty is very 
difficult to estimate for the future LHeC, but we consider the
assumption of 0.5\,\% to be rather conservative.
In order to address the unknown size of the uncorrelated
uncertainty in some detail, we consider in the following two alternative scenarios, one
with an uncorrelated uncertainty of 0.5~\%, as well as one with a more
optimistic value of 0.25\,\%.
These will be denoted in the following as the `a' or 'b' scenarios, respectively.
...
Infact, the effect of a possibly finer binning may be simulated to a very good
...
The properties of the thus generated four sets of data samples are
...
The data can be considered essentially as a collection of cross 
section ratios. In previous studies (see, 
...
extraction procedure to exploit implicitly the corresponding 
information. We can then expect that most of the correlated 
uncertainties, e.g.\ normalization errors, become largely 
...
above will help us to verify this rough estimate of expected 
...
An overall very similar methodology has previously been used by H1 for
the determine of the expected uncertainties in their electroweak analysis of
...
electroweak parameters, are in agreement with dedicated PDF
...
angle is defined by the ratio of the gauge boson masses and used 
as an abbreviation only. At higher orders, there is in addition 
...
not sensitive by itself to higher-order corrections beyond NLO, but
only the actual values would be.
...
is only small\,\footnote{A determination of \mw\ in the
...
which is related with the on-shell definition of \sw\
...
uncertainties, and have been used elsewhere to study the 
