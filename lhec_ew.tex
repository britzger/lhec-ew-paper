...
integrated luminosity will allow us for the first time to perform 
...
Refs.~\cite{Ahmed:1994fa} {\color{red} \em more!} 
and, as an example, a  
...
(FCC) could be extended with an electron-hadron facility, the 
FCC-$eh$~\cite{Abada:2019lih}, with even higher center-of-mass 
energies than at the LHeC. At lower energies, the planned and 
approved Electron Ion Collider 
(EIC)~\cite{Accardi:2012qut,Aschenauer:2017jsk}
\footnote{See also 
  \url{https://wiki.bnl.gov/eic/upload/EIC.Design.Study.pdf}
  } 
could extend DIS experiments towards higher luminosities and 
heavier nuclei. In figure~\ref{fig:dSigmaFacilities} we show  
...
LHeC~\cite{AbelleiraFernandez:2012cc} and earlier for the 
...
been studied elsewhere~\cite{todo,add,references}. The possible 
improvement in our knowledge of parton distribution functions 
due to LHeC experiments was described in \cite{nochwas}
...
input~\cite{Jegerlehner:2011mw,Jegerlehner:1985gq}, since the 
...
is mediated by gauge boson exchange with space-like momentum 
...
these ratios explicitly, but leave it to the fit to extract 
the corresponding information. We can then expect that 
correlated normalization uncertainties cancel to a large 
extent and uncorrelated uncertainties are reduced by taking 
the properly weighted average of all data. As a consequence, 
uncertainties at the per mille level can be expected for the 
observables which we are going to study in the sequel. 
...
considerd fixed. The weak mixing angle is defined by the 
...
The measurement at the LHeC will improve the precision of the 
...
We will now consider $\sin^2\theta_{\textrm{W},f}^\textrm{eff}$ 
as a free parameter which is allowed to vary in a PDF+EW fit, 
but the same for lepton and quark vector couplings. SM higher-order 
...
through DIS cross section measurements for the four scenarios. 
...
self energies are particularly important since they are universal. 
If the masses of the non-SM particles are large, a low-$q^2$ 
expansion of the self-energy corrections, 
\begin{equation}
...
couplings~\footnote{From the combined measurements of 
  ...
  and $\kappa_{\ell}$ have been determined. 
...
The $\rho_\text{NC}^\prime$-$\kappa^\prime$ fit can be interpeted 
as a simultaneous determination of 
...
with those of the coupling constant fits described below in  
...
unexplored kinematic regime of space-like momentum transfer.
...
to the sensitivity in the space-like regime of momentum transfer. 
...
space-like regime of about $25<\sqrt{\Qsq}<700\,\GeV$. 
...
measurement of cross sections and their ratios can therefore 
...
higher-order EW corrections, first of all the top-quark mass \mt. 
The value of \mt\ can be determined with an uncertainty of down to 
...
\au, \vd\ and \ad) or for the the anomalous form factors, 
...
LHeC can not replace other measurements. In order to perform 
a comparison of parameters determined from different observables, 
...
measurements of these mass parameters. The strength of the LHeC 
