...
accelerator and detector design of the LHeC~\cite{Bruning:2706220}. In both cases its kinematic 
reach extends to much 
higher scales in comparison to HERA, which together with the hugely increase of the expected 
...
XXX Daniel, Hubert, I don't know why you want to have so much text on EIC and FCC-eh when there is no single eweak result from either of them.
There are sin2theta prospects for EIC, between NuTeV and the Z-pole.
Further, the FCC-he is far in the future but its parameters are not uncertain. The remarks about nuclei are of no concern here. and there is no need to tell what the EIC is (not) good for. The EIC luminosity is higher than that of HERA but not much higher than 
NMC and BCDMS'. I thus think this paragraph should be very much shortened, and believe we should not make comments on this completely strange outcome of an unclear "strategy" - suggest we discuss this on Thursday XXX
...
FCC-$eh$~\cite{Abada:2019lih}, and will reach still higher 
center-of-mass energies than the LHeC. At lower energies, the 
Electron Ion Collider 
(EIC)~\cite{Accardi:2012qut,Aschenauer:2017jsk,EICpreCDR}
will perform DIS measurements with higher luminosities than was achieved at HERA and including electron scattering off 
...
of the strong interaction and the nucleon 
structure\,\footnote{However, it is 
  expected, that also EIC may contribute to electroweak
  physics as well~\cite{Zhao:2016rfu}.
}.
The realization of the FCC-$eh$ is in the far future and its 
configuration and beam parameters are quite uncertain. The LHeC, 
however, might be realized during the lifetime of the LHC and could 
...
XXXThe present paper is a long write-up of the description of 
the electroweak physics potential in the forthcoming publication of 
the update~\cite{Bruning:2706220} 
of the 2012 Conceptual Design Report of the LHeC,
...
\begin{table}[thb!]
 ...
 \end{tabular}
  \caption{
    ...
    points (see Tab.~\ref{tab:datasets}). 
  }
  \label{tab:scenarios}
\end{table}
...
as a collection of cross section ratios. 
XXXX I do not understand the ratio remark, as the fit is on all cross sections, I doubt there is any magic related to the number of data points XXXX
...
uncertainties, become largely constrained by the fit
...
QCDNUM~\cite{Botje:2010ay,Botje:2016wbq}. The strong coupling 
...
Eqs.~\eqref{eq:gV-LO} and~\eqref{eq:gV-NLO} (cf.\ also next section), 
whereas the contribution from the NC and CC normalization,
Eqs.~\eqref{eq:kappaZ-LO} and~\eqref{eq:deltar-NLO}, and from
the $W$-boson propagator term in CC DIS, $(1/(\Qsq+\mw^2))^2$, cf.\ Eq.~\eqref{eq:cc-cs},
...
\begin{figure}[tb!]
  \centering
  \includegraphics[width=0.54\textwidth]{plot_mWmZ_mitGf}
  \caption{
...
%  \begin{tabular}{lcr@{$\,\pm\,$}lr@{$\,\pm\,$}lr@{$\,\pm\,$}lr@{$\,\pm\,$}l}
  \begin{tabular}{lcllll}
    \toprule
    \Qsq value & Bin $i$ & \multicolumn{4}{c}{Expected relative 
                          uncertainty of
                          $\sin^2\theta_{\textrm{W}}$} \\
    \cmidrule(lr){3-6}      
...
measurements sensitive the \sweffl\ would be useful for further 
improvements.
...
corresponds to 0.65\,\%, as shown in Tab.~\ref{tab:SM-summary}.
...
complimentary with measurements in $e^+e^-$ or $pp$ collisions, 
...
We would like thank S.~Schmitt and A.~Sch\"oning for many related
discussions during the earlier stages of this work.
We thank J.~Erler and R.~Kogler for valuable discussions on the
interpretation of $Z$-pole data, and S.~Kluth for cotinuous support
