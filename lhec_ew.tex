...
A single fit to the LHeC data is performed, where all the four
...
\begin{figure}[tbhp]
  ...
  \caption{
    ...
    The LHeC expectations are compared with results from the combined LEP+SLC experiments~\cite{ALEPH:2005ab}
...
smaller than the currently most precise ones~\cite{PDG19}.
With $E_e=60\,\GeV$ and/or with more optimistic assumptions on the
data, these prospected uncertainties are even further decreased (see tab.~\ref{tab:couplings}, not displayed in fig.~\ref{fig:couplings}).
...
without any (mirror-)ambiguity, since DIS is mediated in the $t$-channel and particularly
...
subsequent section.
({\color{red}also refer to $sin^2\theta_W$ from previous section.})

In NC interactions the form factors $\rho_{\text{NC},f}$ and
$\kappa_{\text{NC},f}$ represent the universal
...
by introducing the anomalous parameters \rhop\ and
\kapp{}~\cite{Spiesberger:2018vki}, which can  
also be considered to be flavor-specific and \Qsq\ dependent:
\begin{align}
...
{\color{red}11. May: As discussed, the determination of
  $\sin^2\theta_\textrm{W,eff}^{\ell}$ represents a particular fit
  of $\kappa^\prime$ and the result can be re-calculated and
  interpreted such that it can be compated with other measurements.
} 

A very common electroweak precision observable in neutral-current
interactions is the leptonic  effective weak mixing angle.
...
the second term in eq.~\eqref{eq:gV} ({\color{red} forward ref. to be fixed.}) according to
\begin{equation}
...
fig.~\ref{fig:sw2eff}.
For LHeC-60b an uncertainty of
\begin{equation}
...
sect.~\ref{sec:rhokappa} below, which is often referred to as the
...
%
Higher-order EW corrections to the CC DIS
...
study two anomalous form factors
$\rho^\prime_{\text{CC},(eq/e\bar{q})}$~\cite{Spiesberger:2018vki},
...
For the LHeC-60b scenario, even smaller uncertainties are be achieved.
...
%-----------------------------------------------------------------------
\clearpage
\section{A global EW fit including LHeC inclusive DIS data and sensitivity to the top-quark mass}
