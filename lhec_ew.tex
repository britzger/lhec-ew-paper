...
(LHeC)~\cite{AbelleiraFernandez:2012cc,Bruning:2019scy,Bruning:2706220} planned at the
LHC may complement the proton ring with an electron beam, allowing 
to perform deep inelastic scattering (DIS) with electrons and 
protons up to TeV energies. Its kinematic reach to higher scales 
in comparison to HERA and the large expected integrated luminosity will 
allow us for the first time to perform high-precision electroweak 
measurements at high scales in DIS.  
...
Refs.~\cite{Ahmed:1994fa} {\color{red} \em more!} and, as an example, a  
final study of electroweak physics with the complete set of 
H1 data was presented in~\cite{Spiesberger:2018vki}. 

Apart from the LHeC, other options for experimentation with 
DIS are presently studied. A possible Future Circular Collider 
(FCC) could be extended with an electron-hadron facility, the FCC-$eh$~\cite{Abada:2019lih}, 
with even higher center-of-mass energies than LHeC.
At lower energies, the planned and approved 
Electron Ion Collider
(EIC)~\cite{Accardi:2012qut,Aschenauer:2017jsk,https://wiki.bnl.gov/eic/upload/EIC.Design.Study.pdf}
could extend DIS experiments towards  
higher luminosities and heavier nuclei. 
In figure~\ref{fig:dSigmaFacilities} we show  
...
\begin{figure}[t!hb]
  ...
  \caption{
    ...
    neutral and charged current $e^-p$ DIS at HERA, LHeC 
    and FCC-$eh$ and the EIC. 
  }
  \label{fig:dSigmaFacilities}
\end{figure}

The realisation of the FCC-$eh$ is in the far future, while the 
LHeC might be realized during the lifetime of the LHC and could 
start taking data in the 2030s. The LHeC has recently been 
described as a realistic option in the EPPSU deliberation 
document~\cite{European:2720131}. Therefore we focus our studies 
in this article on the potential of a future LHeC collider. Studies 
of electroweak effects for similar energies have been performed
in the past for the LHeC~\cite{AbelleiraFernandez:2012cc}  and earlier 
for the LEP$\otimes$LHC proposal \cite{Jarlskog:1990bk}. 
\footnote{The main physics motivation of the EIC lies in studies 
  of the strong interaction and the nucleon structure, but it is 
  expected to contribute to electroweak physics as well 
  \cite{Zhao:2016rfu}. 
}
  % <<<<<<< HEAD
% =======
% {
% The newly proposed energy-recovery linac for a high-quality 
% electron beam, together with the high-luminosity upgrade of 
% the LHC (HL-LHC), are expected to provide an extraordinary 
% increase of the integrated luminosity, and consequently a 
% signigicant increase in the reach towards higher \Qsq\ compared 
% to what was assumed in all previous studies. This motivates us 
% to perform this exploratory study for the LHeC and investigate 
% the new possibilities for the measurement of electroweak physics 
% effects. 
% 
...
% space-like momentum transfer,  where the interaction is mediated 
% by gauge boson exchange in the $t$-channel, is often complementary 
% to other experiments, such as proton-proton collisions or 
% electron-positron annihiliation, or experiments at lower energies, 
...
% been studied elsewhere {\color{red} references !}
% 
...
% parametrize new physics beyond the SM and investigate the 
% precision that can be obtained for limits on corresponding  
...
% convention to generalize the SM gauge-boson fermion copulings 
...
% extreme kinematic reach of the LHeC will also allow us to 
% study the scale-, i.e.\ \Qsq-dependence of coupling parameters, 
% an opportunity which is in fact unique to the LHeC. 
% Finally, we conclude and summarize the most important results 
% in Sec.~\ref{sec:conclusions}. Some details of our fit results 
% are relegated to an appendix (\ref{app:other}). We also discuss 
% the impact of the LHeC measurements on possible future global 
% fits of the electroweak SM parameters to an appendix 
% (\ref{app:globalfit}). 
% 
% 
% {\color{red}
...
% }
% >>>>>>> 8cf3e75481b38c69fc534c7244c7cb02aaf18b28

The newly proposed energy-recovery linac (ERL) for a high-quality 
electron beam, together with the high-luminosity upgrade of 
the LHC (HL-LHC), are expected to provide an extraordinary 
increase of the integrated luminosity, and consequently a 
signigicant increase in the reach towards higher \Qsq\ compared 
...
at the LHeC with exclusive final states, for example $W$- or 
$Z$-boson production, or production of the Higgs boson, has 
been studied elsewhere~\cite{todo,add,references}.
...
In addition, we will study a number of possible ways to 
generically parametrize new physics beyond the SM and investigate the 
precision that can be obtained for limits on corresponding  
new parameters not present in the SM. In Sec.~\ref{sec:stu} 
we study the well-known $STU$-parameters which describe new 
physics entering through loop insertions in the self energy 
...
to the NC and CC cross section. Higher-order EW corrections are defined 
...
have to be provided as independent input~\cite{Jegerlehner(?),todo}, since the 
...
theory predictions depending on the fit parameters. The covariance 
...
fundamental parameters of the SM. At LO these are the masses 
of the weak gauge bosons, $m_W$ and $m_Z$, or the weak mixing 
angle. At higher orders, there is in addition a sensitivity 
...
as an indirect determination of these SM mass parameters. We 
...
possibilities to take one of the masses as a free fit 
...
figure~\ref{fig:mW} and compared with the values obtained 
...
in the left panel of figure~\ref{fig:mW2}. Scenarios LHeC-60a 
...
data. In the right panel of figure~\ref{fig:mW2} we show how 
...
\end{figure}
 ...
 %DB {\color{blue}(For the conclusions: 
 %DB \color{red}
 %DB The determination of \mW\ from $t$-channel processes in  
 %DB DIS provides an interesting alternative to the measurement 
 %DB of \mW\ from $s$-channel, i.e.\ resonant, $W$-production in 
 %DB $pp$ scattering. 
 %DB }
 %DB {\color{brown}
 %DB Measurements at the LHeC will improve the precision of the 
 %DB \mW\ determination obtained by H1~\cite{Spiesberger:2018vki} 
 %DB ($\mW(\text{H1}) = 80.520\pm0.115~\GeV$) by more than an order 
 %DB of magnitude.
 %DB }
 %DB {\color{magenta}
 %DB The determination of the $W$-boson mass from LHeC data 
 %DB will be among the most precise measurements and will significantly 
 %DB improve the world average value of \mw, which is currently 
 %DB dominated by the ATLAS measurement~\cite{Aaboud:2017svj}.
 %DB }
 %DB 
%--------------------------------------------------------
%  Z mass
...
(cf.\ Eq.~(\ref{eq:gV})). In the SM, one has $\kapp{f} = 1$. 
...
\end{figure}
 ...
 %DB  {\color{red} {\em move this to the section for rho-kappa fit: }
 %DB  In a simultaneous determination 
 %DB  of $\sin^2\theta_{\textrm{W},f}^\textrm{eff}$ and a universal 
 %DB  modification of the normalization of NC weak couplings by 
 %DB  \rhop\ as defined in the previous section, we find an 
 %DB  uncertainty of $\Delta \sin^2\theta_{\textrm{W},f}^\textrm{eff} 
 %DB  = \pm 0.00023$ ($\pm0.00071$) for LHeC-60b (LHeC-50a). 
 %DB  } 

The measurement of $\sin^2\theta_{\textrm{W},f}^\textrm{eff}$ 
can be performed in sub-regions of the wide kinematic range of 
\Qsq\ accessible at the LHeC. We find that 
$\sin^2\theta_{\textrm{W},f}^\textrm{eff}$ can be determined in 
...
by boson exchange with space-like momenta, i.e.\ the scale is 
given by $\mu^2 = - Q^2$ (c.f.\ Sect.~\ref{sec:sw2eff}). If a 
...
%  \begin{tabular}{lcr@{$\,\pm\,$}lr@{$\,\pm\,$}lr@{$\,\pm\,$}lr@{$\,\pm\,$}l}
  \begin{tabular}{lcccc}
    \toprule
    & \multicolumn{4}{c}{Expected relative uncertainty of 
  ...
  \caption{
    ...
    $\sin^2\theta_{\textrm{W},f}^\textrm{eff}$ as a function  
    of \Qsq. The uncertainties are obtained in a simultaneous 
    fit of all 12 parameters of 
    $\sin^2\theta_{\textrm{W},f}^\textrm{eff}(\mu^2_i)$ with 
    $\mu_i^2 = - Q_i^2$ together with the PDF parameters. 
  }
\label{tab:sw2Q2}
\end{table}
...
highh since these parameters enter at tree level through the 
...
on the top-quark mass \mt\ \footnote{Note that,
  ...
  However, these indirect contibutions to the inclusive NC/CC 
  DIS cross sections are very small at the LHeC. In particular, 
  the sensitivity to the actual value of the top-quark mass can 
  be neglected.
}.
...
expect to observe a sizable effect on the DIS cross sections. 

We have determined the uncertainties of the top-quark mass \mt\ 
through DIS cross section measuremants for the four scenarios. 
...
0.6\,\GeV\ and is already included in the above values. In these 
studies, the value of \mW\ is considered as an external, fixed  
...
$\Delta\mW = \pm 12\,\MeV$~\cite{PDG19}. This corresponds to 
...
in the range between $\pm0.5$ and $\pm1.0$\,\GeV (see 
Ref.~\cite{PDG19}, and references therein).
...
higher-order EW corrections to DIS cross sections corresponds 
to the one used in the calculation of observables which enter 
...
simultaneous determination of \mt, \mw, and the PDFs is displayed 
for selected LHeC scenarios in figure~\ref{fig:mWmt} and compared 
...
from NC/CC DIS data. \mH\ also enters through the self-energy 
corrections in the SM, however, the \mH\ dependence is only 
logarithmic, $\propto\log(\mH^2/\mw^2)$, i.e.\ very weak. 
A PDF+EW fit with the Higgs-boson mass as a free parameter 
leads to an uncertainty of $\Delta m_H=^{+29}_{-23}$ and 
$^{+24}_{-20}~\GeV$ for the scenarios LHeC-50a and LHeC-60b, 
respectively. This compares well with the precision found in 
the indirect \mH\ determinations from LEP+SLC combined 
...
Extensions of the Standard Model with new particles commonly 
lead to modifications of predictions through new contributions 
to higher-order corrections. Loop insertions with 
particle-antiparticle pairs in gauge boson self energies can 
become relevant even if the new particles are too heavy to be 
produced directly in scattering processes. If the masses of the 
non-SM particles are large, a low-$q^2$ expansion of the 
...
Results for various PDF+$STU$ fits are presented in 
...
The PDF+$STU$ fit to LHeC DIS data in the on-shell scheme 
can be combined with the contraint from the $G_F$ measurement. 
...
$U$ is modified. Results of a PDF+$STU$ fit in the modified 
...
couplings are modified by new physics. A systematic way to 
parametrize modified weak NC couplings is based on using   
anomalous parameters \rhop\ and \kapp{}. The first, \rhop{}, 
...
\label{eq:gA} 
  ...
  \left( \Itf - 2 Q_f \kappa_{f}\kapp{f}\sw \right) \, . 
\label{eq:gV} 
\end{eqnarray}
...
modification of the weak mixing angle $\sin^2\theta_W$ (see 
section \ref{sec:sw2eff}). 

We determine the uncertainties of the anomalous coupling 
parameters $\rho_\text{NC}^\prime$ and $\kappa^\prime$ 
...
couplings~\footnote{In the LEP+SLD 
  ...
  $\kappa_{\ell}$ have been determined. 
...
with those of the coupling constant fits described below in the 
section~\ref{sec:nccouplings} (see Fig.~\ref{fig:couplings}).
...
\begin{table}[bht!]                                                                                                                                                           
  ...
  \caption{ 
    ...
    CC $\rho^\prime$ with the NC $\kappa^\prime$ parameter. 
  }
  \label{tab:rhokappap}
\end{table}                                                                                                                                                                   
...
triggers. These features allows one to measure the hadronic 
...
\hline
  ...
  \caption{
    ...
    scenarios, obtained in a simultaneous PDF+EW fit. The upper 
...
% --- one general concluding sentence
The proposed LHeC experiment at CERN's HL-LHC
will provide a very unique opportunity for high precision
electroweak physics
%DB {\color{brown}
  in neutral-current (NC) and
  charged-current (CC) interactions
  % }
  in a yet completely unexplored kinematic regime of
space-like momentum transfer.

% --- technical summary
In this article we have explored simulated inclusive
NC and CC deep-inelastic scattering (DIS) cross
section data at electron-proton center-of-mass energies of 1.2 and
1.3\,TeV, and assessed their sensitivity to a number of parameters of
electroweak theory and the sensitivity to possible generic extensions
of the theory.
% (DB: swap following two sentences?)
Our theoretical calculations include next-to-next-to-leading order QCD
corrections to the DGLAP evolution and structure function calculation
and the full set of one-loop electroweak corrections to the
electron--(anti-)quark $t$-channel scattering diagram in the on-shell
renormalization scheme for NC and CC exchange.
%
The simulated pseudo-data have been provided by the LHeC Study Group
and comprise a full set of experimental systematic uncertainties and
are used elsewhere for prospects for determinations of parton
distribution functions.
The latter are implicitly also included in our analysis framework,
while we extend the phenomenological analysis towards electroweak parameters.



% --- SM parameters
The sensitivity of inclusive NC/CC DIS data at the LHeC to the input{\color{red}(?)}
parameters of electroweak theory in the Standard Model formalism are
summarized in table~\ref{tab:SM-summary}.
%As the fine structure constant $\alpha$ and the Fermi coupling constant are
%measured with extraordinary precision at higher scales, ...
%
\begin{table}[bht!]
  \footnotesize
  \centering
  \begin{tabular}{lcccccc}
    ...
    \midrule
    \mw+PDF      &  \mw & MeV  &  $\pm6$ & $\pm10$ & $\pm12$ & $\pm8$ \\
    \addlinespace
    ...
    \addlinespace
    log\mH+PDF      & $\log\tfrac{\mH}{\GeV}$ &   &  $\pm0.11$ & $\pm0.17$ & $\pm0.13$ & $\pm2.0$ \\
    log\mH+PDF      &    \mH & GeV  &  $^{+14}_{-13}$ &  $^{+24}_{-20}$  &  $^{+17}_{-15}$ &  $^{+28}_{-23}$ \\
    \bottomrule
  \end{tabular}
  \caption{
    Prospects for the determination of Standard Model parameters from
    simulated inclusive NC and CC DIS data at the LHeC.
    Scenarios for electron beam energies of $E_e=50$ and 60\,\GeV, and
    for two assumptions on the resolution and uncertainties, denotes
    as scenarios `a' and `b', are studied.
  }
...
% ... W-mass
We find, that in the on-shell renormalization scheme the $W$-boson mass parameter \mw\
can be determined with an experimental uncertainty of up to
$\pm6\,\MeV$.
The high experimental precision is obtained from the large kinematic
range of the NC/CC DIS data and the related large amount of data points that
all contribute to the sensitivity in the space-like regime of DIS.
The high precision will improve over all present measurements
and will provide a highly valuable validation of future improved
direct \mw\ measurements with \MeV\ accuracy.

% ... sweff2
The leptonic effective weak mixing at $+\mz^2$, \sweff, can be determined with
an experimental uncertainty of up to $\pm1.5\cdot10^{-4}$, while only
small and well-known \Qsq\ and flavor dependent correction factors
from theory need to be taken into account for its determination.
As such{\color{red}(?)}, the measurement at LHeC will be superior than present
measurements at hadron colliders.
% ... sw2 running
In addition, the \Qsq\ dependent LHeC DIS data allow for a unique determination of the
\emph{running} of the effective
    %DB {\color{red}(?)}
     weak mixing angle in
the space-like regime of about $25<\sqrt{\Qsq}<700\,\GeV$ and small
experimental uncertainties below $\pm20\cdot10^{-4}$ are achieved in
the range of $60\lesssim\sqrt{\Qsq}\lesssim450\,\GeV$.

% mt and mH
%DB{\color{brown}
  The high precision of the born-level parameters of EW theory also
  suggest a determination of
%DB }
the dominant parameters of the 
higher-order EW corrections, the top-quark mass and the Higgs-boson mass,
\mt\ and \mH.
The value of \mt\ can be determined with an uncertainty of up to
$\pm1.1\,\GeV$ and thus provides a precise determination which is
complementary to direct measurements as it is theoretically clean and
free from ambiguities of QCD theory.
As the Higgs-boson mass contributes only logarithmically, the
uncertainties are fairly large and compare in size with the indirect
determinations from the LEP+SLD data.


% --- global fit(s)
A simultaneous determination of the weak boson masses, \mw\ and \mz,
or a simultaneous determination of \mw\ together with \mt, yield
moderate uncertainties. These, however, compare well with nowadays
achieved uncertainties in global EW fits elsewhere.
We have then studied in a simplified formalism the potential impact of
LHeC NC/CC DIS data to such a global EW fit and found only moderate
improvements, which is because the same \mW--\mt\ and
\mw--\mz\ relations are accessed in DIS than in higher-order EW
corrections to other observables.
Therefore, inclusive NC/CC DIS cross sections at the LHeC will
determine the minimum set of input parameters of
electroweak theory altogether with moderate precision only, but these data rather
probe particular aspects of electroweak interactions, and at high accuracy.
A few such studies have been performed.


% --- STU
Non Standard Model modification to the weak boson self energies can be
considered in a first order $q^2$ dependent approximation by three
\emph{oblique} parameters, commonly denoted $S$, $T$ and $U$.
Through the simultaneous analysis of NC and CC DIS data, the $STU$ 
parameters can be determined indpedendently, and uncertainties on $S$,
$T$ or $U$ of up to a few percent are achieved.

% --- rho kappa
Modifications of the $\textrm{U}(1)\times \textrm{SU}(2)$ gauge structure
%DB {\color{red}(?)}
may alter in NC interactions the vector and
axial-vector couplings of fermions to the $Z$.
We have therefore introduced flavor and \Qsq\ dependent anomalous
form-factors as multiplicative modifications of the SM form-factors
$\rho_{\text{NC},f}$ and $\kappa_f$.
These can be determined with uncertainties up to a
few permille and compare in size well with comparable results from LEP+SLD.
%DB {\color{brown}
In contrast, however, LHeC has high sensitivity to
electroweak interaction of light quarks.
%DB }
The \Qsq\ dependence of the anomalous form factors, $\rhop{f}$ and
$\kapp{f}$, can be determined with percent precision in the range
$60\lesssim\Qsq\lesssim600\,\GeVsq$.

% --- further tests
The weak NC couplings of fermions, \vf\ and \af, can be considered in a
rather model-independent manner as a free parameter of a model, while
their measured values may then be compared to the expectations from EW theory.
While these couplings have been measured at the $Z$-pole with high
precision, in particular for leptons and $b$ quarks, the vector and
axial-vector couplings of light quarks, $u$ and $d$, are
experimentally measured only with poor precision.
LHeC will provide a unique
measurement with permille accuracy of these important quantities (\vu,
\au, \vd\ and \ad).
As such, with LHeC, for the first time, electroweak interactions of
the lightest quarks will be probed with highest precision.


% --- CC
In literature, mainly studies of the NC sector of EW theory are
discussed, while CC interactions at high scales are expected to obey
the GSW theory.
Due to the unique experimental conditions in DIS, where the CC
kinematic observables can be completely measured from initial and
final-state event quantities, at the LHeC also studies of the CC
sector of EW theory can be performed in a unique manner.
Similarly as for NC, we have introduced \emph{ad hoc} anomalous
modifications to the electron--quark and electron--anti-quark
scattering diagram.
These anomalous form factors can be determined with an accuracy up to
$\mathcal{O}(0.1\,\%)$, and also their potential \Qsq\ dependence can
be measured with permille accuracy up to about $\sqrt{\Qsq}\lesssim400\,\GeV$.
...
% --- final words
%
In conclusion, the high center-of-mass energy, the large kinematic range, and the
large integrated luminosity at the LHeC will allow for the first time
precision electroweak measurement at high scales in NC and CC DIS.
%
Rich aspects of electroweak theory can be
probed with highest precision, while these measurements will not be
limited by the need for parton distribution functions.
In many cases, the results are complementary to direct measurements in
$e^+e^-$ or hadron-hadron collisions, such as the determination of the
$W$-boson mass or the top-quark mass.
Outstanding results are expected for: scale-dependent measurements such
as the running of the weak mixing angle or scale-dependent non-SM
parameters; studies of weak interactions of the light quarks; and
experimental studies of charged current interactions. 


 %DB \clearpage
 %DB     {\color{blue} Old sentences...}
 %DB     
 %DB The space-like momentum transfer in DIS further allows for unique
 %DB scale dependent test of the electroweak theory, both, for neutral and
 %DB charged currents.
 %DB %This includes for instance scale dependent measurements of the effective
 %DB %weak mixing angle in the range of about $40<\sqrt{\Qsq}<700\,\GeV$
 %DB %with a precision up to a few permille.
 %DB 
 %DB 
 %DB In summary, the inclusive NC and CC DIS cross sections measured at
 %DB LHeC provide a unique opportunity to perform high-precision
 %DB determinations of fundamental electroweak parameters and test the
 %DB quantum  nature of electroweak processes with high precision.
 %DB 
 %DB Unique measurements will be made for weak charged currents and
 %DB for the scale-dependence of electroweak interactions.
 %DB 
 %DB {\color{red}We studied in a traditional way NC and CC separately.
 %DB Many models predict changes to NC and CC simultaneously. A such, a
 %DB simultaneous determination of NC and CC observables is reasonable}
 %DB 
 %DB {\color{blue} Write about what we have NOT done... \\}
 %DB Further direct measurements, as for instance Higgs production,
 %DB top-quark production or single $W$ or $Z$
 %DB production cross sections, or vector-boson-scattering cross sections,
 %DB as well as heavy flavor cross sections in NC and CC DIS, will provide further
 %DB improvements for electroweak precision measurements.
 %DB 
 %DB 
 %DB {\color{brown} 
 %DB   Refer to improvement of LHC measurements due to precise LHeC PDFs?
 %DB }
 %DB 
 %DB 
...
\gf\ and the SM masses from the PDG16~\cite{Olive:2016xmw} we obtain 
...
fit~\cite{Haller:2018nnx,deBlas:2016ojx,Erler:2019hds} and confirms that the 
simplifications of our approach are justified for our purpose. 
...
%  \includegraphics[width=0.47\textwidth]{plot_lhec_mWmt_global_2.pdf} 
  ...
  \caption{
    ...
    including LHeC-60b inclusive NC/CC DIS data.
    The results are compared with expected uncertainties from the simplified 
...
in the fit as well, and we use recent values for \mW, \mt, and \mH\ from PDG20~\cite{PDG20}.
When including LHeC-60b NC/CC DIS data, we obtain a very moderate
improvement for \mw\ from $\Delta\mw=\pm4.6\,\MeV$ to $\Delta\mw = \pm 4.5\,\MeV$.
The 68\,\% C.L. contours in the \mW-\mt--plane are displayed in
Fig.~\ref{fig:globfit}.
An improvement to \mt\ is only seen beyond the rounding digit, which
is reasonable, since \mt\ contributes to NC/CC DIS only through
higher-order corrections.
Obviously, already other than the LHeC NC/CC DIS data 
are sufficient to provide good constraints on \mW\ and \mt. 
LHeC DIS data probe essentially the same relation of \mW\ and \mt\ 
and they cannot compete with direct measurements of these 
mass parameters.
The strength of the LHeC EW analysis is its 
...
PDF determination.
LHeC data will offer, for the first time, 
the possibility to study a simultaneous fit of EW and PDF 
parameters. The impact of the latter may be considerably 
reduced if measurements at the LHeC will become available 
\cite{Azzi:2019yne}.
%
Commonly, the global fit is repeated with direct measurements of the
parameters of interest excluded, i.e.\ without \mw\ and \mt.
...
\begin{table}[bht!]
  ...
  \begin{tabular}{lcccc}
    ...
    \mw, \mt, \mZ, \mH             & \mw, \mt  &   $\pm4.6$ & $\pm0.63$  &  $+0.83$ \\
    \mw, \mt, \mZ, \mH, NC/CC DIS (LHeC-60b)  & \mw, \mt, PDF  &   $\pm4.5$ & $\pm0.63$  & $+0.83$ \\
    \bottomrule
  \end{tabular}
  \caption{
