...
the first electron proton collider, 
{\color{brown} 
a much larger range of momentum transfer squared, $Q^2$, came 
into reach -- an important prerequisite for studies of the 
electroweak interactions in DIS}. First measurements of
electroweak effects at HERA have been undertaken in 
Refs.~\cite{Ahmed:1994fa,Aktas:2005iv} and more complete 
electroweak analyses have been performed only recently,
...
%
%{\color{blue}
%  DB: shall we add a short comment about the figure?
%  Something like:
%  For studies of electroweak effects, particular data at higher
%  scales/\Qsq are important
%  }
%
\begin{figure}[t!hb]
  \centering
...
structure{\color{brown}\st{, and due to its
...
$Q_{e/q}$, in units of the positron charge, and on the third 
component of the weak-isospin of the fermion, $I^3_{{\rm L},e/q}$. 
They are given, at LO, by\,\footnote{In the following, 
  we will use the subscripts $q$, $\ell$ and $f$ to refer 
  generically to specific quark or lepton flavors or both, 
  respectively, 
  {\color{red} but also to universal, i.e.\ flavor indpendent, 
  quantities ???. The meaning becomes clear from the context.}}
\begin{eqnarray}
  g_A^{f} 
  &=& I^3_{{\rm L},f}
\label{eq:gA-LO} \,, \\
...
\caption{
  ...
  sources used in the simulation of the NC 
  {\color{brown} and CC} cross sections.
...
$\overline{D}_A$, $\overline{D}_B$, $\overline{D}_C$). The values 
of the PDF 
parameters used for the generation of pseudo data are not of 
particular relevance here. They have been obtained from a 
private fit to HERA data, similar to 
Refs.~\cite{Aaron:2012qi,Andreev:2017vxu}. 
...
DIS data. 
{\color{brown} 
In this initial part of our study we are, in particular, 
...
\begin{figure}[tb!]
  ...
  \caption{
    ...
    The scenarios LHeC-60a and LHeC-50a (uncorrelated uncertainty of
    0.5~\%) and LHeC-60b and LHeC-50b (0.25~\%) are marked by vertical
    lines. 
    %The scenarios `a' and `b' are indicated by vertical lines.
    Right: The uncertainty $\Delta m_W$ as a function of 
    the size of the normalization uncertainty of the DIS cross sections.
...
for cross section ratios\,\footnote{ 
  ...
  of a per mille for \st{dedicatedly constructed} cross section 
  ratios. Such cross section ratios are determined by coefficients 
  containing \sw\ (see Eqs.~\eqref{eq:kappaZ-LO} 
  and~\eqref{eq:cc-cs}). 
  }
  Simple error propagation allows one to 
  infer $\Delta m_W / m_W = (\sw/2\cos^2\theta_W) (\Delta \sw /\sw)$.
  Therefore a factor of about $\sw/2\cos^2\theta_W \simeq 0.15$
  {\color{brown}
  applies if the relative uncertainty of the cross section ratios
  is translated into an uncertainty of \mw.
  } 
  This results in a relative uncertainty of 
  $\Delta\mw/\mw\simeq\mathcal{O}(10^{-4})$. 
}. 
The two scenarios (`a' and `b') differ by the assumption for the 
size of the single-bin uncorrelated uncertainty, but have otherwise
{\color{brown} \st{equivalent} similar} 
experimental uncertainties. The dependence 
...
%
We conclude that the LHeC 
can be expected to yield a $W$-boson mass determination with 
the smallest experimental uncertainty from a single 
experiment.
It will even be superior to the current world average.
Therefore, when real data are available, a detailed 
...
of the SM mass parameters. In fact, 
{\color{brown} 
the philosophy of this indirect parameter determination is similar 
to the one of the so-called }
`global EW fits'~\cite{Haller:2018nnx,deBlas:2016ojx,Erler:2019hds},
...
{\color{brown}
The `measurement' of \mw\ from inclusive DIS cross sections at 
the LHeC, therefore, provides a consistency check of the 
SM and is complementary to direct, true, mass measurements of 
the $W$-boson mass. 
} 

%{
%  \color{red}DB: Drop or put elsewhere:
%  \color{brown}
...
{\color{brown} {\em remove:}
(cf.~Eqs.~(\ref{eq:gV-NLO}, \ref{eq:sin2w-eff}))}, 
\begin{equation}
  \sweffl
  = 
  \kappa_{\ell}(\mZ^2) \sw \,. 
...
{\color{brown} cf.\ Eq.~(\ref{eq:sin2w-eff}),}
\begin{equation}
  \sweff(\mu^2) 
  = 
  \kappa_{f}(\mu^2) \sw \,.
...
We will now consider \sweff\ as a free parameter which is allowed 
to vary in a \sweff+PDF fit, but is the same parameter for lepton 
and quark vector couplings. SM higher-order corrections are taken 
into account as described in Sec.\ \ref{sec:theo} by keeping the 
\Qsq- and flavor-dependent form factors $\kappa_{f}$ 
(see Eq.~(\ref{eq:gV-NLO}))
{\color{red}
  ????? ... in order to translate the fit parameter to the
  actual quantities used in the calculation (?)}.
Our estimate for the uncertainties 
...
{\color{brown} leptonic} 
weak mixing angle {\color{brown} \st{expressed in terms of 
leptonic $\sweffl(m_Z^2)$}}. 
%
\begin{figure}[tb!]
  \centering
  ...
  \caption{
    ...
    {\color{brown} the effective weak mixing angle. 
    The results from 
...
may enter when the LHeC measurements are mapped to the leptonic 
...
as the LEP+SLD combination. Even measurements 
...
large contributions from pure photon exchange at lower \Qsq, which is 
independent of the weak mixing angle. 

{\color{red}DB: Here we have a problem: for sw2-running, we have labelled the plot \sw,
  but in the table we have \sweff. This needs to become consistent.
  }
\begin{figure}[tb!]
  \centering
...
%  \begin{tabular}{lcr@{$\,\pm\,$}lr@{$\,\pm\,$}lr@{$\,\pm\,$}lr@{$\,\pm\,$}l}
  ...
  \caption{
    ...
    \sweff\ as a function of \Qsq\ for the four LHeC 
    {\color{brown} \st{studies}}
    scenarios.
    The uncertainties are obtained in a simultaneous 
    fit of all 12 parameters 
    $\sin^2\theta_{\textrm{W},f}^\textrm{eff}(\mu^2_i)$
    with $\mu_i^2 = - Q_i^2$ ($i=1,\dots,12$) together with the 
    PDF parameters. 
    {\color{brown} \st{Absolute uncertainties $\Delta\sweff$ may 
    be trivially calculated by
...
{\color{blue} \st{DB: Add motivating sentence?}} 
\\
...
values of $m_Z$ and $m_W$.
{\color{red} 
\st{DB: does all of the following apply to the OS fits? 
{\color{brown} Yes} }} 
Single-parameter fits of $S$, $T$ 
...
In the modified on-shell scheme, additional constraints on the $STU$
parameters may be obtained by adding further direct measurements of \mw\ or \sweffl,
e.g.\ from measurements in $e^+e^-$ or hadron-hadron collisions.
The parameter relations of such measurements are also
indicated in Fig.~\ref{fig:STU_MOMS} and in particular external
measurements sensitive the \sweffl\ would be useful for further improvements.
...
electron as predicted by the SM.
{\color{red}Motivating sentence?}
We perform a fit of the four 
...
couplings of the second- {\color{brown} and third-}generation 
...
\begin{table}[bht!]
  ...
  \caption{ 
    ...
    CC parameter $\rhopW{}$ (see Eqs.~(\ref{eq:rhocc1}-\ref{eq:rhocc4}) 
    in Sec.~\ref{sec:ewcc}) with the NC $\kappa^\prime$ 
    parameter. 
  }
  \label{tab:rhokappap}
\end{table}                                                                                                                                                                   
...
and high-bandwidth triggers{\color{brown}; 
particularly important is the fact that} in CC DIS the event 
kinematics can be fully reconstructed from the measurement 
of the hadronic final state and the incoming electron beam 
four-{\color{brown}\st{vector}momentum}.
...
0.3\,\%.
For the LHeC-60b scenario, even smaller 
uncertainties can be achieved and we find in 1-parameter+PDF fits
relative uncertainties below one per mille.

{\color{brown} \em Remove this: }
{\color{red}
  Discuss \rhopW{,f}+\kapp{f}+PDF here. Probably also move
  numerical results from Tab.~\ref{tab:rhokappap} to
   Tab.~\ref{tab:rhoCC}??
  . . . At LEP+SLD, there are $\rho$+$\kappa$-fits, but their
  interpretation in DIS is ambigous, because $\rho$ can be 
  $\rho_{NC}$ or $\rho_{CC}$...
}
...
therefore only a moderate impact on the size of the uncertainties 
...
{\color{brown} \st{(NC and CC)}} in a yet 
completely unexplored kinematic regime of spacelike momentum 
transfer. 
...
are summarized in Tab.~\ref{tab:SM-summary}.
At the LHeC, the high experimental precision for SM parameters 
is {\color{brown} due to the fact that a large kinematic range 
of space-like momentum transfer can be used to obatin a large 
amount of DIS data points that all contribute 
to the sensitivity\st{, because DIS is mediated by 
spacelike momentum transfer}}. 
For our prospects, we perform combined fits of EW
parameters and PDFs to simulated NC and CC DIS data, while
from error propagation and simple considerations about ratios
of cross sections data points, e.g.\ NC over CC or high-\Qsq\ over
low-\Qsq\ data points, one expects uncertainties of similar magnitude.
 ({\color{red}...not discussed as such in text...}).
{\color{red}...improve/shorten...
\color{brown}
\st{The large number of cross section data in different $Q^2$ bins 
and including both NC and CC scattering, as well as scattering 
with positrons and with polarized electrons,
is equivalent t
a large number of cross section ratios.}
\st{Therefore, correlated 
...
Typical cross section ratios 
which carry the information about electroweak parameters are 
ratios of NC cross sections at large and small $Q^2$.
These observables are sensitive to the normalization of the weak 
relative to the electromagnetic interaction, i.e.~to the 
weak mixing angle.
Also CC over NC ratios contain similar 
information, being proportional to $1/\sin^4\theta_W$.}
}
{\color{red}DB: the ratios hold only for the determination of mw or
  sin2, but not for sin2 in the vector couplings....} 
Therefore, we conclude that for instance the weak mixing angle 
can be determined with a high experimental uncertainty of down to 
$\pm1.5\cdot10^{-4}$ which corresponds to 0.65\,\%, as shown in
Tab.~\ref{tab:SM-summary}.
The {\color{brown} large number \st{amount} of data points} at 
different \Qsq\ will also allow a determination 
...
center-of-mass energies, $\sqrt{s}=1.2$ and 1.3\,\TeV, 
{\color{brown} \st{referring} corresponding}   
to an electron beam energy of 50 or 60\,\GeV, respectively.
{\color{brown} \st{We} 
Our studies }confirm the presumption that higher center-of-mass 
energies would be quite valuable for the precise determination 
of EW parameters.
% DB: 
% ..., although the extension of \Qsq\ dependent studies with higher $\sqrt{s}$ is only small.
% DB: or:
Of course, with higher $\sqrt{s}$, also the range of \Qsq\ dependent
studies can be {\color{brown} \st{somewhat} extended 
to some extent.}
%
Further improvements will be obtained by higher resolution of the
detector, 
{\color{brown} which would allow us to obtain measurements for 
a larger set of independent data points}. 
Altogether, the expected uncertainties from the four LHeC scenarios
studied in this article differ by about a factor of 2 to 3.
Since many parameters are measured with comparable uncertainties in
$e^+e^-$ or hadron-hadron interactions such an improvement may be
very valuable from a more global {\color{brown} point of} view. 
{\color{red} ???}
...
angle have to be matched to each other
%like parameter relations 
%between the weak boson masses and the weak mixing angle, or when
%measurements of the effective leptonic weak  
%mixing angle from $e^+e^-$ observables are to be compared 
%with \sweff\ determined from DIS at a level of 
%$\mathcal{O}(10^{-4})$ or below.
%We did not take into account 
%that additional theoretical, parametric uncertainties play 
%a role.
We believe that our study provides a motivation for 
the investigation and calculation of higher-order corrections 
to DIS observables beyond the 1-loop level, and in different
...
{\color{red} 
for the first time ???}, the possibility to study a simultaneous 
fit of EW and PDF parameters. The impact of the latter may be 
