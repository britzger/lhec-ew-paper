...
we will analyze pseudo data simulated with
different assumptions on the experimental uncertainties or the center-of-mass energy.
Measurements in the regime of 
...
higher-order electroweak corrections in the following Sec.~\ref{sec:theo}.
...
with which SM parameters are expected to be determined. {\color{red}(?)} 
...
fundamental parameters of EW theory from LHeC inclusive DIS data, and 
in this initial study put an enhanced focus on the
estimates of the uncertainties of the simulated data.

Since our analysis is based
on theory predictions derived in the on-shell scheme, the 
free parameters at the LO are only the masses of the weak gauge bosons,  
$m_W$ and $m_Z$, and the fine structure constant $\alpha$.
The latter is fixed in the analysis, i.e.\ it is considered to be
known with ultimate precision.
The weak mixing angle is defined by the 
ratio of the gauge boson masses and used as an abbreviation 
only.
At higher orders, there is in addition a sensitivity 
to the top-quark and the Higgs-boson mass, which will be 
studied in a subsequent section only. 

We determine the expected uncertainties for \mw\ in an \mw+PDF fit,
where the value of \mz\ is considered as an external input, e.g.\ from
the LEP+SLD combined measurement~\cite{ALEPH:2005ab}.
%
%We study the expected uncertainties 
...
%external input. 
%
For the $W$-boson mass parameter, we then find 
...
Altogether, we find a relative uncertainty for \mw\ of the order of $10^{-4}$,
which is compatible with our rough initial estimate for cross section
ratios\,\footnote{ 
  In the previous section we outlined that mainly due to the large
  number of data points one expects relative uncertainties
  of a per mille for dedicatedly constructed cross section ratios.
  Such cross section ratios are {\color{brown}determined by
    coefficients}/{\color{blue}proportional to coefficients} containing
  \sw\ (see Eqs.~\eqref{eq:kappaZ-LO} and~\eqref{eq:cc-cs}). Simple error 
  propagation allows one to infer
  $\Delta m_W / m_W = (\sw/2\cos^2\theta_W) (\Delta
  \sw / \sw)$.
  Therefore a factor of about
  $\sw/2\cos^2\theta_W \simeq 0.15$
  applies to the relative uncertainty of the cross section ratios
  if those are translated into an uncertainty of \mw. This results in
  a relative uncertainty of $\Delta\mw/\mw\simeq\mathcal{O}(10^{-4})$. 
}.
% new paragraph here??
\begin{figure}[tb!]
  \centering
...
equivalent experimental uncertainties. The dependence 
...
Fig.~\ref{fig:mW} and compared with the measurements\,\footnote{In
  ...
  applies to the PDG world average.} % \mW\ values and uncertainties obtained 
...
in-depth discussion of its interpretation and the relation to other,
more direct, measurements:
The precise measurement of DIS cross sections yield primarily 
(only) indirect determination of the SM mass parameters.
Infact, the results of these mass parameter determinations can be
interpreted similarly to the philosophy of the so-called 
...
The `measurement' of \mw\ from inclusive DIS cross sections at the
LHeC, therefore, rather provides a consistency test of the SM, if the
value of \mw\ is compared to direct mass measurements, than
a direct measurement of the mass of the weak boson itself.
...
assume the weak mixing angle in the fermion neutral-current couplings as a free 
...
at the $Z$ pole~\cite{ALEPH:2005ab}. The results are conventionally expressed in 
...
\begin{figure}[tb!]
  ...
  \caption{
    ...
    LHeC-60b.
    The SM expectation is displayed as dotted line.
  }
\label{fig:sw2Q2}
\end{figure}
%
...
in the range between $\Delta \mt = \pm0.3$ and $\pm2.0$\,\GeV~\cite{LHCTopWG} 
...
{\color{blue}DB: Add motivating sentence?\\}
...
from the LHC experiments nowadays~\cite{Sirunyan:2020xwk,ATLAS-CONF-2020-005}. 
...
{\color{red}DB: does all of the following apply to the OS fits?}
...
can be combined with the constraint from the $G_F$ measurement, cf. Fig.~\ref{fig:STU_OS}. 
...
\begin{figure}[tb!]
  ...
  \caption{
    Same as Fig.~\ref{fig:STU_OS}, but the calculations are performed in the
    modified on-shell scheme:
    The value for $m_Z$ is fixed, but the $W$ boson mass is calculated from its relation to 
    the Fermi constant \gf.
    The relations how direct measurements of \mw\
    or \sweff\ at the $Z$-pole would constrain the two oblique
    parameters are additionally indicated by dashed and dotted lines,
    respectively.
    In the modified on-shell scheme, the measurement of \mw\ would
    constrain the same relation as a mesurement of $1-\mw^2/\mz^2$
    (dashed line).
  }
  \label{fig:STU_MOMS}
\end{figure}
...
state and the incoming electron beam four-vector.
...
{\color{red}
  ...
  numerical results from Tab.~\ref{tab:rhokappap} to Tab.~\ref{tab:rhoCC}??
  . . . At LEP+SLD, there are $\rho$+$\kappa$-fits, but their
  interpretation in DIS is ambigous, because $\rho$ can be $\rho_{NC}$
  or $\rho_{CC}$...
}
...
$s$ and $b$), denoted as $g_V^{u}$, $g_V^{d}$ and $g_A^{u}$, $g_A^{d}$, 
...
The resulting uncertainties of the fits are summarized in Tab.~\ref{tab:couplings}
...
neutral- and charged-current interactions (NC and CC) in a yet completely 
...
At the LHeC, the high experimental precision on SM parameters is
achieved from the large kinematic range of the DIS data and the
related large amount of data points that all contribute to the
sensitivity, because DIS is mediated by spacelike momentum transfer.
...
to $\pm6\,\MeV$.
This is compatible with a rough estimate
{\color{brown}, based on simple error propagation,}
of an expected relative uncertainty at the level of $10^{-4}$.
This high precision will improve all present 
measurements and provide a highly valuable validation of future 
improved direct \mw\ measurements with $\mathcal{O}(\MeV)$ accuracy.
...
%
We have compared LHeC inclusive DIS pseudo data at
two different center-of-mass energies, $\sqrt{s}=1.2$ and 1.3\,\TeV, referring to
an electron beam energy of 50 or 60\,\GeV, respectively.
We confirm the presumption, that higher center-of-mass energies would
be quite valuable for the precise determination of EW parameters.
% DB: 
...
studies can be somewhat extended.
%
Further improvements will be obtained by higher resolution of the
detector, that allow for the measurement of more independent data points.
...
very valuable from a more global view.


% --- pre-final words
In this study, we employed calculations in the on-shell
renormalization scheme and including the full set of one-loop EW corrections.
For our main purpose of uncertainty prospects, this provides a
complete parameter sensitivity, also from higher-order contributions.
However, once real LHeC data is available a more careful study of
higher-order corrections is necessary.
On the one hand, this is to match the high experimental precision of
the data with accurate theoretical predictions.
%
On the other hand, a conclusive test of the SM can only be achieved by 
comparing as many as possible different measurements,
i.e.\ also observables in processes other than DIS, 
and therefore a careful study of higher-order corrections is necessary.
For example, different definitions of effective parameters have to be
matched to each other, like parameter relations 
...
with \sweff\ determined from DIS at a level of $\mathcal{O}(10^{-4})$
or below.
...
to DIS observables beyond the 1-loop level{\color{brown}, and different
