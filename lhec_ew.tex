...
\begin{table}[hbt]
  ...
  \begin{tabular}{lccc}
    \toprule
    Scenario name & $E_e$ & $\delta_\textrm{unc}$ & Bin grid \\
    \midrule
    LHeC-50a  &   50\,\GeV  &  0.5\,\%  &  nominal \\
    LHeC-50b  &   50\,\GeV  &  0.25\,\% &  4$\times$ nominal  \\
    LHeC-60a  &   60\,\GeV  &  0.5\,\%  &  nominal \\
    LHeC-60b  &   60\,\GeV  &  0.25\,\% &  4$\times$ nominal \\
    \bottomrule
 \end{tabular}
\caption{
  LHeC scenarios.
...
A more meaningful test of the high-energy behaviour of EW
...
behaviour of EW theory at high energies, while only
...
represented by the  $\rho$ and $\kappa$ parameters and the
correction factor $\Delta r$.
...
with LHeC at $E_e=50\,\GeV$ the top-quark mass can be determined 
with an uncertainty of $1.8$\,--\,$2.2\,\GeV$, dependent on the
size of the uncorrelated uncertainties of the data
($\delta_\textrm{unc.}=0.5\,\%$ or 0.25\,\%,
c.f.\ sect.~\ref{sec:data}). 
In the scenario with $E_e=60\,\GeV$, the top-quark
mass is determined with an uncertainty of $1.4\,\GeV$ or
\begin{equation}
  \Delta\mt= \pm1.1\,\GeV\,
\end{equation}
for the two assumptions on $\delta_\textrm{unc.}$.
...
nowadays known with an uncertainty of $\Delta\mW=12\,\MeV$\,\cite{PDG}, and
...
about 0.5\,--\,1.0\,\GeV 
(see Ref.~\cite{PDG}, and references therein).
...
Therefore, this measurement provides an important cross check of the top-quark
mass in EW dominated processes.
Infact, this top-quark mass measurement compares well to \emph{global EW}
...
In the optimistic  scenario with $E_e=60\,\GeV$ and
$\delta_\textrm{unc.}=0.25\,\%$ an uncertainty contour similar to the
...
intereference and $Z$ exchange parts of the NC cross sections, see
eq.~\eqref{eq:gV-LO}.
...
Howeve, the up-type and down-type couplings can be well interpreted as the
...
dominate the PDFs. 
Only for the down-type couplings relevant contributions may arise from
...
for different LHeC scenarios, and  results for LHeC with
$E_e=50\,\GeV$ are compared to the current most 
precise measurements in figure~\ref{fig:couplings}.
%
\begin{figure}[tbhp]
    \centering
...
With $E_e=60\,\GeV$, and/or with more optimistic assumptions in the
...
predictions. {\color{red}Values are displayed in tab.~\ref{tab:couplings}}


At the LHeC, the vector and axial-vector couplings can be disentangled
without any ambiguity, since DIS is mediated in the $t$-channel and also
the $\gamma Z$ contributions are of great importance.
This is  different to measurements at the $Z$ pole, where only the
squares of the couplings are measured, which then yields three `mirror
...
theory as the theory of weak interactions of the light quarks, and
...
\begin{figure}[htbp]
  ...
  \caption{
      ...
      Results from LEP+SLC~\cite{ALEPH:2005ab}, Tevatron~\cite{Aaltonen:2018dxj},
...
size than the world average or the LEP+SLC combination. 
...
Studies on the CC form factor $\rho_\text{CC}$, are studied in the
subsequent section.
...
$\kappa_{\text{NC}}$ represent the \Qsq dependent  universal
...
Such modifications can be described conveniently by introducing the
anomalous parameters \rhop\ and \kapp{}, 
