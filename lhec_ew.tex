...
\begin{figure}[t!]
  ...
  \caption{
    ...
    {\color{red} use only PDG20 - check values!}
    }
    \label{fig:mW}
\end{figure}
...
(cf.\ Eq.~(\ref{eq:gV})). 
%A determination of the weak mixing 
...
are shown in Fig.~\ref{fig:sw2Q2} and Table~\ref{tab:sw2Q2}. 
We emphasize that DIS is complementary to other measurements 
since the scattering process is mediated by boson exchange with 
space-like momenta, i.e.\ the scale is given by $\mu = 
- \sqrt{Q^2}$ (c.f.\ Sect.~\ref{sec:sw2eff}). If a cacluation 
of DIS cross sections including higher-order EW corrections in 
the $\overline{\rm MS}$ scheme is available, the uncertainty of 
this \Qsq-dependent \sw-determination can be translated into a 
test of the running of the weak mixing angle. 
...
results of a global fit. We describe in the following a 
simplified approach which is suited for our purpose. 
...
$\sin^2\theta^\textrm{eff}_{W,\ell}$, %  $\sigma^0_\textrm{had}$,
partial decay widths and asymmetry observables in $e^+e^-$ 
collisions~\cite{ALEPH:2005ab} $R_\ell^0$, $R_b^0$, $R_c^0$,
$A_\ell$, $A_c$, $A_b$, $A_\textrm{FB}^{0,\ell}$, 
$A_\textrm{FB}^{0,c}$, $A_\textrm{FB}^{0,b}$, QCD contributions 
to the QED vacuum polarisation $\Delta\alpha_\textrm{had}^5$, 
and sometimes also measurements of $\alpha_\textrm{s}$, $m_b$, 
and $m_c$. With the assumption that $\alpha$ and \gf\ are known 
with ultimate precision, the fit parameters are chosen as 
\mZ, \mH, \mt, $\Delta\alpha_\textrm{had}^5$ and
$\alpha_\textrm{s}$. Sometimes also $m_b$ and $m_c$ are taken 
as parameters to be determined by the fit. 
...
{\color{red} ???
In a rational view, and since the fit is overconstraint, 
a global EW fit is a trade-off between the experimental 
uncertainties of the measurements and the uncertainties
of their predictions that arise in turn also from the 
uncertainties of the measurements (if theoretical uncertainties 
are ignored).
}
We observe that some of the fit parameters are determined with 
...
only with a much larger uncertainty. In the case of \mW\ and 
\mt{}, the uncertainties from direct measurements is of a 
similar size as those from a global fit. All other observables 
have much larger experimental uncertainties than predicted by 
the global fit. 
{\color{red} ??? 
Consequently, the constraints from a global EW fit arises 
predominantly from the comparison of the measurement of \mW\ 
and \mt\ with their predictions, which are obtained from the 
precision measurements of $\alpha$, \gf, and \mz\ (with subleading 
contributions from $\log\mH$, $m_b$, $m_c$, 
$\Delta\alpha_\textrm{had}^5$).
} 

Therefore, we perform a simplified global EW fit, where we 
consider only the {\color{red} measurements} of \mZ, \mW, \mH\ 
and \mt. All other parameters do not provide additional 
constraints or are known with good precision 
{\color{red} for their sub-leading relevance.} 
The calculations are performed in the on-shell scheme.
The fit parameters are chosen to be \mW, \mH\ and \mt, 
while \mz\ is a {\color{red} prediction}\,\footnote{
  {\color{red}Note.} Even simpler, and equally
  good: Input measurements are \mZ, \mW, \mt, and fit parameters 
  are \mW, \mt. 
  }.
When using {\color{red} above measurements} as input~\cite{PDG19} 
we obtain uncertainties of
$\Delta\mw=\pm5.6\,\MeV$ ($\pm6.2\,\MeV$) and 
$\Delta\mt=\pm0.87\,\GeV$ ($\pm2.3\,\GeV$) when the 
{\color{red} respective} 
measurements are included (excluded) in the fit.
This result is very similar to the one of thegeneral global 
EW fit~\cite{Haller:2018nnx,deBlas:2016ojx} and confirms that 
the simplifications of our approach are justified for our purpose. 
...
\begin{figure}[t!]
  ...
  \caption{
    Left: Simplified global EW fit including LHeC-60b inclusive 
    NC/CC DIS data. The results are compared with the simplified 
...
in the fit as well. We then obtain a moderate improvement for 
the uncertainty of \mW\ to $\Delta\mw = \pm 5.5\,\MeV$.
The 68\,\% C.L. contours in the \mW-\mt--plane are displayed in
Fig.~\ref{fig:globfit}. Obviously, already other data do already 
constrain \mW\ and \mt\ already very well, and LHeC cannot
compete with direct measurements of \mz\ or \mH, and 
{\color{red} ??? the strong
correlation of \mW\ with \mt\ (c.f.\ fig.~\ref{fig:mWmt}) 
prevents a relevant impact of LHeC data on these parameters 
in such fits.
}

{\color{red}\em The following remarks can go to the conclusions:}

The strength of the LHeC EW analysis is its complementarity with 
measurements in $ee$ or $pp$ or particular aspects of the EW 
theory, as for instance light-quark couplings to the $Z$.
Further relevance is gained, since the dominant uncertainty in
$W$-boson mass measurements or measurements of
$\sin^2\theta^\text{eff}_W$ at the LHC (or HL-LHC) are PDF 
related uncertainties. These may be considerably reduced with 
LHeC~\cite{Azzi:2019yne}.
