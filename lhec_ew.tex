...
\label{eq:last2}
\end{eqnarray}
Higher-order pertubative corrections of QCD can be included by 
using $Q^2$-dependent parton distribution functions, evolved 
according to the DGLAP equations 
{\color{red} Dokshitzer etc + reference}. 
In addition, there are corrections of order $O(\alpha_s)$ to 
the relations (\ref{eq:last1}, \ref{eq:last2}) and the 
longitudinal structure function is a prediction. 
...
electromagnetic, interaction. It is calculated as
\begin{equation}
  \varkappa_Z(\Qsq)
  = \frac{\Qsq}{\Qsq+m^2_Z}
...
Using $\sw = 1-m_W^2/m_Z^2$, they are given by
\begin{eqnarray}
  g_A^{f} 
...
$\kappa_{\text{NC}, f}$ are unity. At higher orders, they 
are promoted to form factors representing universal higher-order 
corrections. Since they depend on \Qsq, they render the coupling 
constants `effective' running couplings. 
...
\label{eq:cc-cs}
\end{equation}
In the naive quark-parton model the structure functions $W_2^\pm$ 
and $xW_3^\pm$ are obtained from parton distribution functions, 
while $W_{\rm L}^\pm = 0$. An incoming electron can scatter only 
with positively charged quarks, therefore one has 
\begin{equation}
  W_2^- =
...
\label{eq:w23po-LO}
\end{equation}
Here, we have used the combinations of parton distribution 
functions $U = u+c$, $\overline{U} = \bar{u} + \bar{c}$, 
$D = d+s$ and $\overline{D} = \bar{d} + \bar{s}$.
{\color{red} 
Add sentence about higher-order QCD corrections, reference? 
}

{\color{red} 
\paragraph{Add} paragraph(s) about higher order corrections: 
\Qsq dependent form factors, etc. Scheme dependence. 
In this study we adopt the on-shell scheme using \mz\ 
and \mw\ as input parameters for the calculations. 
{\em How much really? Refer to \cite{Spiesberger:2018vki} ? }
} 
...
while the CC cross section is suppressed relative to the NC  
cross section. At values of \Qsq below the mass of the $W$ 
...
%    \cmidrule(lr){7-8}
  ...
  \caption{
    ...
    and 60~\GeV.
    {\color{red} \em Remove last two columns? Explain the 
    meaning of No. of data points!}
    {\color{blue}to be checked, whether Ee=50 and 60 
    used these settings.}
  }
  \label{tab:datasets}
\end{table}
...
measurements of NC and CC DIS cross sections up to values 
...
is assumed. Another data sample will be collected with a 
...
an integrated luminosity of 1\,fb$^{-1}$ is assumed.
All data sets are restricted to $\Qsq\geq5\,\GeVsq$ in order 
...
instance, in Ref.~\cite{AbdulKhalek:2019mps} (where, however, 
slightly different assumptions on the data uncertainties 
{\color{red} \em important ???}
have been used).
...
\caption{
  ...
  propagate 
  {\color{red} directly ???} to the cross sections.
  %which are transported to
  %provide correlated systematic cross section errors. 
  {\color{red} 
  The last four items are uncertainties by various sources and 
  directly assigned to the cross section. 
o  ??? }
}
\label{tab:uncert}
\end{table}
...
The data include 
{\color{red} a full set of systematic uncertainties ???} 
and
...
is assumed. The statistical uncertainty is taken to be 
{\color{red} at least ???}  0.1~\%. Finally, a global 
normalisation uncertainty of 1~\% is taken into account, 
which includes the luminosity uncertainty. 

A number of {\color{red} further ???} potential uncertainty 
sources is combined in an uncorrelated uncertainty component 
of 0.5\,\%. 
...
grid for the simulated data points was chosen, but real data 
may allow a much finer binning, in particular at medium $x$ 
values or at higher \Qsq. 
{\color{red} Insert here: No. of data points / bins?}
With 
{\color{red} 
the given treatement ???} 
of fully correlated, or fully uncorrelated uncertainties, 
...
where the grid in both $x$ and \Qsq is chosen finer by a 
factor of 2. This can simply be emulated by choosing the 
uncorrelated uncertainty correspondingly improved by the 
same factor, i.e.\ equal to $0.25\,\%$.

The statistical uncertainty is determined by the expected 
number of events; however, we keep in any case a minimum value 
of 0.1~\% for every data point.

The properties of the four sets of data samples which we will 
use are collected in Table\,\ref{tab:scenarios}.
...
\begin{table}[t!]
 ...
 \end{tabular}
  \caption{
    ...
    referred to by the names shown in the first column. The 
    last column indicates 
    {\color{red} 
    the technical realisation in the data tables of the chosen 
    bin grid. ???}
    {\color{blue}
      An increased number of bins w.r.t.\ the \emph{nominal} number of
      data points (c.f.\ table~\ref{tab:datasets}) is emulated by re-calculating
      the uncorrelated uncertainty accordingly, i.e.\ for `nominal'
      $\Delta_\textrm{uncorr}=0.5\,\%$ is used and for `4$\times$
      nominal' a value of $\Delta_\textrm{uncorr}=0.25\,\%$ is used.
    }
  }
  \label{tab:scenarios}
\end{table}
...
$\bar{D}_A$, $\bar{D}_B$, $\bar{D}_C$). 
The actual values of the PDF parameters are not of particular 
relevance here, and are set to values, as they can be obtained 
from a fit to HERA data~\cite{Aaron:2012qi,Andreev:2017vxu}. 
{\color{red} ???}
...
values {\color{red} equivalent ???} to the predictions,
i.e.\ our data represent an \emph{Asimov data set}
\cite{Cowan:2010js}. It it is important to note that with the 
above definition of $\chi^2$ the actual value of the cross 
section at a given point does not enter the calculation of 
the uncertainties, but only the relative size of the
...
in \dr\ will have to be taken into account. The estimate of 
experimental and PDF uncertainties given above, is, however, not 
sensitive by itself to higher-order corrections beyond NLO. 

{\color{red} ???
An uncertainty of the top-quark mass of $0.5\,\GeV$ will yield 
an additional uncertainty of $\Delta\mW=2.5\,\MeV$. 
???} 
...
\begin{figure}[t!]
  ...
  \caption{
    ...
    (PDG19)~\cite{Tanabashi:2018,PDG19}. 
    \\
    {\color{green} Shift label at the x-axis of the left plot, 
    numbers missing?}
    }
    \label{fig:mW}
\end{figure}
...
Figure~\ref{fig:mWmZ}, where the 68\,\% confidence level
contours are displayed. The precision of these two mass 
parameters in a simultaneous fit is only moderate. 
However, a {\color{red} 
very meaningful test of the high-energy behaviour of EW
theory at a much higher precision ???} 
can be performed by imposing the additional constraint for 
the precise measurement of $G_F$~\cite{Tishchenko:2012ie}
(see \cite{Brisson:1991vj,Spiesberger:1993jg}). 
As a result, a very shallow ellipse is obtained, due to the
very high experimental precision of the $G_F$ measurement.
{\color{red}
Such a fit simultaneously determines and tests the behaviour 
of EW theory at high energies in a single experiment and
process, while only low-energy parameters $\alpha$ and $G_F$ 
are taken as inputs. ???}
...
{\color{magenta}
Dicsussion about scheme dependence of parameters? 
No, not here: is irrelevant for the uncertainties. 
}
...
lepton charge. 
{\color{red} \em is this correct? is something else important?} 
In fact, these effects due to the weak interaction are 
important predominantly at higher values of \Qsq, corresponding 
...
up- and down-quarks, i.e.\ of light quarks only. 
{\color{red}
Only for the down-type couplings, a contribution from strange 
quarks may have a significant size. ??? 
}
{\color{red} \em Does up-type / down-type include charm / bottom?
}

We perform a PDF+EW fit where the vector and axial-vector 
couplings of up-type and down-type quarks, i.e.\ $g_V^{u}$, 
$g_V^{d}$ and $g_A^{u}$, $g_A^{d}$, are free parameters in 
one single fit. The fit parameters are identified with the 
coupling constants defined in the Born approximation, and
\Qsq-dependent higher-order corrections are calculated in
the SM formalism in the 1-loop approximation. The resulting 
uncertainties are summarized in table~\ref{tab:couplings}
for different LHeC scenarios. Figure~\ref{fig:couplings} 
shows the results for the LHeC-50a scenario, compared with 
the current most precise measurements. 
...
\begin{figure}[b!]
  ...
  \caption{
    ...
    measurements by D0~\cite{D0} and H1~\cite{Spiesberger:2018vki}.
...
%
    $\au$  & $0.50$   & $^{+0.04}_{-0.05}$   & $0.0015$ & $0.0022$ & $0.0027$ & $0.0035$ \\
    $\ad$  & $-0.514$ & $^{+0.050}_{-0.029}$ & $0.0034$ & $0.0055$ & $0.0059$ & $0.0083$ \\
    $\vu$  & $0.18$   & $\pm0.05$            & $0.0010$ & $0.0015$ & $0.0020$ & $0.0028$ \\
    $\vd$  & $-0.35$  & $^{+0.05}_{-0.06}$   & $0.0027$ & $0.0046$ & $0.0059$ & $0.0067$ \\
    \midrule
...
%log.couplings.e.lhec50+.uncorr.0.5.txt:EPRC.ve                   =   -0.0542056   +/-   0.00414468  
    $\gae$  & & & $0.0009$ & $0.0014$ & $0.0018$ & $0.0025$   \\
    $\ve$  & & & $0.0014$ & $0.0022$ & $0.0031$ & $0.0041$   \\
    \bottomrule
\hline
  \end{tabular}
  \caption{
    Present values and uncertainties of the light-quark weak 
    neutral current couplings ($\au$, $\ad$, $\vu$, $\vd$) from 
    the PDG~\cite{Tanabashi:2018,PDG19} and the uncertainties 
    ...
    free fit parameters, 
    {\color{red} the next . . . and the next . . . 
    nothing for LHeC-50b ? }
    {\color{blue} What to do with PDG? This seems to be lost and unrelated. }
    }
\label{tab:couplings}
\end{table}
...
the measurement of these EW parameters; their uncertainties 
can be improved by about an order of magnitude through LHeC 
data. A scenario with $E_e = 60~\GeV$ and/or with more 
optimistic assumptions for systematic uncertainties is 
particularly promising, see table~\ref{tab:couplings}. In this 
table we also show results from fits where only two, or one 
single, coupling is a free fit parameter while the other 
couplings are fixed at their SM value. 
{\color{red} 
An ultimate uncertainty of ... can be achieved with LHeC-60b. 
}
...
{\color{green} 
Shall we estimate an uncertainty on the quark couplings
due to the uncertainty on the electron-couplings? 
} {\color{red} a correlation???}

{\color{red} Insert a short statement about the uncertainty 
for electron couplings. }
...
\Qsq-dependent. A similar study of a generalization of the 
CC form factor $\rho_\text{CC}$ will be discussed below in 
Sect.~\ref{sec:ewcc}. \kapp{} can also be interpreted as a 
...
and $\kappa_\text{NC}^\prime$ parameters. The results are 
displayed in Fig.~\ref{fig:rhokappa:f}. In this figure, we 
compare the expected LHeC uncertainties with corresponding 
results that have been obtained from combined LEP+SLD 
data for leptonic couplings~\footnote{In the LEP+SLD 
  combination the parameters $\rho_{\textrm{NC},\ell}$ and 
  $\kappa_{\textrm{NC},\ell}$ have been determined.
  For this comparison, we have translated 
  {\color{red} \em definitions? how precisely ???}
  the uncertainties
...
\begin{figure}[t!]
  ...
  \caption{
    ...
    with the uncertainties obtained from an analysis of LEP+SLD 
    combined data~\cite{ALEPH:2005ab} for leptonic couplings. 
  }
...
\begin{figure}[thbp]
  ...
  \caption{
    ...
    \rhop\ and \kapp{} for two different LHeC scenarios. 
    In case of LHeC-60, with $E_e=60\,\GeV$, an uncorrelated 
    uncertainty of $0.25\,\%$ on the data points is imposed. 
    {\color{red} \em Clarify: which scenario! Left and right???}
    {\color{blue}
      Highest precision is obtained in the region of about $\Qsq\approx 20\,000\,\GeVsq$ and \rhop\ 50a: 0.0029435, 60b: 0.0014824; \kapp\ 50a: 0.0022532, 60b: 0.00112846.
      }
  }
...
the anomalous couplings. To do this, we perform fits of 
$\rho_\text{NC}^\prime$ and $\kappa_\text{NC}^\prime$ to LHeC 
data split into {\color{red} how many?} subsets with different 
\Qsq-ranges. Our findings are shown in Fig.~\ref{fig:rhokappaQ2} 
for the scenarios LHeC-60a and LHeC-50a, where we include, for 
comparison, results obtained from H1 data~\cite{Spiesberger:2018vki}. 
{\color{red} \em describe results, typical numbers . . . }

{\color{red} \em Move this to the next section: }
{\color{magenta}
The measurement of $\kappa_\text{NC}^\prime(\Qsq)$ can be 
translated to a measurement of the effective weak mixing angle 
and it shows, that the \Qsq dependence of 
$\sin\theta_{\text{W}}^{\text{eff},(\ell)}(\Qsq)$ can be determind
over a wide kinematic range of about $25<\sqrt{\Qsq}<700\,\GeV$
with a precision of up to 0.1\,\% and everywhere better than 
1\,\%. We emphasize that DIS is complementary to other 
measurements since the scattering process is mediated by boson 
exchange with space-like momenta, i.e.\ $\Qsq=-q^2$ with
...
fit parameter while keeping the propagator masses fixed 
{\color{red} \em True? Or what precisely is fixed, $m_Z$, $G_F$?}. 
...
We perform a PDF+EW fit assuming $\sin^2\theta_\textrm{W}$ 
...
(see Eq.~(\ref{eq:gV-LO})). Results for LHeC beam energies 
of $E_e=50~\GeV$ or 60~\GeV are collected in
Fig.~\ref{fig:sw2eff}. For the LHeC-60b scenario, our 
estimate for the uncertainty is 
\begin{equation}
  \Delta \sin^2\theta_\textrm{W} =  \pm0.00015 \, .
\end{equation} 
{\color{red} \em Use same format as for $\Delta m_W$ above: 
split into exp + PDF uncertainties and provide results 
for all scenarios. 
}
This is superior to any current single measurement and of 
similar size as the world average or the LEP+SLD combination. 
Even measurements in a space-like region of momentum transfers, 
i.e.\ for a non-resonant process, turns out to be competitive 
with $Z$-pole measurements, despite of the fact that the cross 
section receives large contributions from pure photon exchange, 
which is independent of the weak mixing angle. 
...
\begin{figure}[t!]
  ...
  \caption{
    ...
    expressed in terms of the leptonic effective parameter 
    ...
    Erler:2019dcx} 
    {\color{red} \em combined by?}
    and the world average~\cite{Erler:2019dcx} 
    {\color{red} and PDG?}
    are all obtained from a 
    combination of various separate measurements (not shown 
    individually) (see also Ref.~\cite{Erler:2019hds} for 
    additional information). 
  }
\label{fig:sw2eff}
\end{figure}
 
{\color{red} \em Move the following paragraphs to somewhere 
else:} 
{\color{magenta}
...
\Qsq-dependent modifications. In a simulataneous determination 
of $\sin^2\theta_\textrm{W,eff}^{\ell}$ and a universal 
modification of $\rho$, an uncertainty 
$\sin^2\theta_\textrm{W,eff}^{\ell}=\pm0.00023$
($\pm0.00071$) for LHeC-60b (LHeC-50a) is found in the
EW+PDF fit. 
{\color{red} \em (this is equivalent to the rho-kappa fit).}
}
{\color{brown}
The sensitivity of the LHeC NC/CC DIS data on \Qsq-dependent SM
modification is studied in a somewhat more general framework in
Sect.~\ref{sec:rhokappa}. If . . . in the MSbar scheme: the 
`running' of the weak mixing angle. 
}
{\color{red} 
Not mentioned so far: $\kappa^\prime_{\text{NC},e}$, and
$\kappa^\prime_{\text{NC},e}+\kappa^\prime_{\text{NC},u} 
+\kappa^\prime_{\text{NC},d}$, and table~\ref{tab:sw2eff}.
}

{\color{blue}Values are:
\begin{eqnarray}
  \Delta\sin^2\theta_\textrm{W,eff}^{\ell} (\text{LHeC-60b}) 
  &=& 
  \pm0.00014_{({\rm exp})}\pm0.00006_{({\rm PDF})}
  =\, 0.00015_{\text{(tot)}}{\rm ~~~~and~~} % 9.6
  \\
  \Delta\sin^2\theta_\textrm{W,eff}^{\ell} (\text{LHeC-60a}) 
  &=& 
  \pm0.00023_{({\rm exp})}\pm0.00009_{({\rm PDF})}
  =\, 0.00025_{\text{(tot)}}
  \\
  \Delta\sin^2\theta_\textrm{W,eff}^{\ell} (\text{LHeC-50b}) 
  &=& 
  \pm0.00017_{({\rm exp})}\pm0.00014_{({\rm PDF})}
  =\, 0.00022_{\text{(tot)}}{\rm ~~~~and~~} 
  \\
  \Delta\sin^2\theta_\textrm{W,eff}^{\ell} (\text{LHeC-50a}) 
  &=& 
...
boson. Similarly as in the NC case, these are represented in 
terms of the form factors $\rho_{\text{CC},eq}$ and 
$\rho_{\text{CC},e\bar{q}}$. 
{\color{red} \em should be introduced already in the theory 
section.}
To allow for physics beyond 
the SM, we introduce new anomalous, primed parameters,  
$\rho^\prime_{\text{CC},\, eq}$ and  
...
%    \rhopW{,e\bar{q}}+PDF  & \rhopW{,e\bar{q}}   &  1  &   0.0030  & 1 & 0.0012  & 1 & 0.0012  & 1 & 0.0012 \\
  ...
  \rhopW{,eq} and \rhopW{,e\bar{q}}.
  {\color{red} note: these numbers need to be updated, 
  if we want to include such a table.}
  }
\label{tab:rhoCC}
\end{table}
...
%  \begin{tabular}{lcr@{$\,\pm\,$}lr@{$\,\pm\,$}lr@{$\,\pm\,$}lr@{$\,\pm\,$}l}
  \begin{tabular}{lcccccccc}
    \toprule
    & & \multicolumn{4}{c}{Expected uncertainty} & \multicolumn{3}{c}{Correlation} \\
    \cmidrule(lr){3-6}    \cmidrule(lr){7-9}
    Fit parameters & Parameter & LHeC-60b& LHeC-60a& LHeC-50b & LHeC-50a & S & T & U \\
    \midrule
    S+PDF      &  S  &  $0.05$ & $0.07$ & $0.06$ & $0.06$ \\
    T+PDF      &  T  &  $0.02$ & $0.03$ & $0.02$ & $0.04$ \\
    U+PDF      &  U  &  $0.02$ & $0.03$ & $0.04$ & $0.03$ \\
    \addlinespace
    S+T+U+PDF  &  S  &  $0.25$ & $0.39$ & $0.58$ & $0.74$ & 1.0 & 0.8 & -0.6 \\
               &  T  &  $0.32$ & $0.42$ & $0.51$ & $0.65$ &     & 1.0 & -0.9\\
               &  U  &  $0.30$ & $0.38$ & $0.37$ & $0.46$ &     &     & 1.0 \\
    \addlinespace
    S+T+PDF  &  S  & $0.23$ & $0.33$ & $0.44$ & $0.53$ & $1.0$ & $0.98$  \\
             &  T  & $0.09$ & $0.14$ & $0.19$ & $0.23$ &       & $1.0$  \\
    \addlinespace
    S+U+PDF  &  S  & $0.19$ & $0.25$ & $0.27$ & $0.32$ & $1.0$ & $0.97$  \\
             &  U  & $0.09$ & $0.12$ & $0.13$ & $0.16$ &       & $1.0$  \\
    \addlinespace
    T+U+PDF  &  T  & $0.24$ & $0.28$ & $0.24$ & $0.28$ & $1.0$ & $-0.99$  \\
             &  U  & $0.28$ & $0.32$ & $0.28$ & $0.33$ &       & $1.0$  \\
    \bottomrule
  \end{tabular}
  \caption{
    STU parameters
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.32\textwidth]{plot_ST}
  \includegraphics[width=0.32\textwidth]{plot_SU}
  \includegraphics[width=0.32\textwidth]{plot_TU}
  \caption{2D-STU fits.}
  \label{fig:STU}
\end{figure}
