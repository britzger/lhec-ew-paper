...
{\color{blue} Outline:
  We first give some introduction, explain the methdology, and
  introduce the LHeC inclusive NC/CC DIS pseudo-data.
  Then we determine the input parameters to the SM theory, \mW\ and
  \mZ\ (that enter the predictions already at the born-level).
  Then we investigate the weak mixing angle in the SM formalism.
  We then consider extensions of the SM in terms of insertions to the
  self-energies due to   additional heavy particles, which are
  commonly parameterised by the $S$, $T$ and $U$ parameters.
  Then we exploit radiative corrections in the SM formalism by
  determining the top-quark or Higgs-boson mass (once compared to direct
  measurements these also probe non-SM contributions to the loop
  radiative corrections).
  Then we consider model independent modifications of the vector and
  axial-vector couplings (\Qsq-dependent, and also for different
  flavors, these are tests of the form factors $\rho$ and $\kappa$).
  Similar studies are done for CC ($\rho^\prime_{\text{CC}}$).
  Then, we study the weak neutral current couplings in a
  model-independent manner.
  Of course, we also conclude in the end.
}
...
%
Extensions of the Standard Model with new particles commonly lead 
to modifications of predictions through new contributions to higher-order 
corrections. Loop insertions with particle-antiparticle pairs 
in gauge boson self energies can become relevant even if the 
new particles are too heavy to be produced directly in 
scattering processes. If the masses of the non-SM particles are 
large, a low-$Q^2$ expansion of the self-energy corrections provides
a sufficiently precise approximation by constant parameters. 
Taking into account that the electromagnetic $U(1)$ gauge 
symmetry has to stay intact and that some of the constants ({\color{blue} constant terms (?)}) 
can be absorbed into renormalization constants, there are 
three free parameters, generally called $S$, $T$ and $U$~\cite{Peskin:1991sw}. A suitable 
definition is described in Ref.\ \cite{PDG2020} which we adopt 
in the following, while their relation to alternative
definitions~\cite{Kennedy:1988sn,Altarelli:1990zd}%,Altarelli:1993sz ,Altarelli:1989hv
...
These fits are performed in the OMS scheme and the SM masses are fixed at
their PDG values, in particular the values of $m_Z$ and $m_W$. 
Single-parameter fits of $S$, $T$ or $U$ can provide uncertainties that are 
better by a factor of 2 to 5 compared to the present 
PDG values~\cite{PDG2020}.
In further fits we observe a very strong correlation of two parameters.
This can be traced back to the fact that only certain 
linear combinations of $S$, $T$ and $U$ contribute to the 
NC and CC scattering cross sections and the $\gamma Z$ 
interference contribution.
For instance, the values of $T$ and $U$ can be disentangled at all only
through their contribution to CC DIS, but not from NC DIS alone.
...
{\color{red}DB. Note. This statement only holds, if the linear
  combinations in $Z$-pole fits would be different than in DIS. Not
  sure, if this is the case...}
  
New physics parametrized with the help of $S$, $T$ and $U$ 
will also affect the \gf-\mw\ relation, 
Eq.~(\ref{eq:deltar-NLO}), through the $W$-boson self energy 
correction to the muon decay. In the modified on-shell scheme~\cite{Marciano:1980pb}, 
where $m_W$ is calculated from $G_F$, new physics will 
therefore not only contribute corrections to the measured 
cross sections, but also through a modification 
of the input parameters. Using the modified on-shell scheme 
corresponds to effectively imposing the additional constraint 
on SM parameters resulting from the \gf\ measurement. As a 
consequence, the sensitivity to $S$, $T$ and $U$ is modified. 
Results of a $STU$+PDF fit in this modified on-shell scheme 
...
case. However, the 2-  and 3-parameter fits 
exhibit weaker correlations leading to smaller uncertainties 
...
\section{Mass parameters from radiative corrections}
While the weak boson masses are important quantities
for the cross section calculation already at the Born-level, the value of the
top-quark mass contributes first at the level of electroweak radiative
correction to the inclusive NC/CC DIS cross sections\,\footnote{Note,
  at sufficiently high scales, e.g.\ $\Qsq\gtrsim2\mt$, the top-quark
  contributes also to the QCD evolution equations and structure
  function calculations. However, these contibution to the inclusive
  NC/CC DIS cross section at the LHeC are very small. In particular the
  sensitivity to the actual value of the top-quark mass can be neglected.}.
As such, a determination of the top-quark mass from inclusive NC/CC
DIS data and its comparison with
direct measurements tests electroweak theory already at the level of
virtual corrections (c.f.\ also subsequent sections).

The dominant corrections to the vertex and to the propagator
self-energies depend on the top-quark mass \mt, and are
represented by the  $\rho_\textrm{NC}$ and $\kappa$
parameters and the correction factor \dr.{\color{red}Use $\rho_t$ here ??}
Therefore inclusive DIS cross sections are sensitive to \mt, and the
leading terms are proportional to $\mt^2/\mW^2$.
%Since possibly other heavy particles may contribute to
...
The top-quark mass \mt\ can be determined from the scenarios LHeC-50a
and LHeC-50b with an uncertainty of $\Delta\mt=\pm2.2\,\GeV$ and $\pm1.8\,\GeV$, respectively.
For the LHeC scenarios with $E_e=60\,\GeV$, the top-quark
mass is determined with an uncertainty of $\Delta\mt=\pm1.4\,\GeV$ (LHeC-60a) or
\begin{equation}
...
In these studies, the value of \mW\ is considered
as an external parameter to the PDF+EW fit.
However, the dominant theoretical uncertainty arises from $\mW$, which is
nowadays known with an uncertainty of
$\Delta\mW=\pm12\,\MeV$\,\cite{PDG19}.
This yields and uncertainty on \mt\ of about $\pm2\,\GeV$.

The size of the LHeC experimental uncertainty compares well with
uncertainties from recent LHC measurements, which are commonly
about $\pm0.5$ to $\pm1.0$\,\GeV 
...
Furthermore, in NC/CC DIS the top-quark mass contributes only through
EW radiative corrections and therefore the determination of $\mt$ is
in a first approximation free from sizeable QCD dependent corrections.
Therefore, the measurement at the LHeC provides an important cross
check of the top-quark mass from purely electroweak effects.
...
uncertainties of $\Delta\mt=\pm2.1\,\GeV$ are achieved~\cite{Haller:2018nnx} .
...
\begin{figure}[th!]
    ...
    \caption{
      Simultaneous determination of the $W$-boson and top-quark masses
      from LHeC-60a or LHeC-50a data.
      As compaison, the results from the $Z$-pole fit with LEP+SLD
      data~\cite{ALEPH:2005ab} and from a global EW fit, where direct
      measurements of \mW\ and \mt\ have been excluded~\cite{Haller:2018nnx}.
    }
    \label{fig:mWmt}
\end{figure}
%
As the leading self-energy and vertex corrections
are proportional to $\mt^2/\mw^2$ it is reasonable to
determine the $W$-boson mass \mw\ simultaneously together with \mt.
Prospects for such a simultaneous determination of \mt, \mw, and
the PDFs is displayed for selected LHeC scenarios in
figure~\ref{fig:mWmt} and compared to results from the LEP+SLD
combination of $Z$-pole measurements~\cite{ALEPH:2005ab} and to a
global EW fit~\cite{Haller:2018nnx}, for which the
direct measurements have been excluded. 
It is found that the uncertainties of the LHeC is
superior to the LEP+SLD combination.
In case of LHeC-60b the uncertainty
contour is even of similar size than the global EW fit.
Since both studies, the global EW fit and the LHeC, exploit the same
$\mt^2/\mw^2$ terms of the radiative corrections, the
\mt--\mw\ correlations are found to be very similar. 

{\color{red} Todo. Is there need to discuss/mention the choice of the
  renormalisation scheme (once \gf is taken as input, there is no \mt-sensitivity in \dr...}

These encouraging prospects for the determination of $\mt$ and $\mw$ may
suggest also a relevant impact of inclusive NC/CC DIS data as
an additional input to a global EW fit (see
e.g.\ Refs.~\cite{deBlas:2016ojx,Haller:2018nnx,PDG19}), which is
studied briefly in appendix~\ref{app:globalfit}.

{\color{brown}
  Once the direct measurements of \mt\ and \mw\ are considered
  in such a global EW fit (in contrast to the results displayed in
  fig.~\ref{fig:mWmt}),
  the $\mt^2/\mw^2$ relation of the radiative
  corrections is resolved and the global EW fit studies the
  dominant parameters of EW theory ($\alpha$, \gf, \mz, \mw, \mt, \mH,
  \dots) at a higher precision~\cite{deBlas:2016ojx,Haller:2018nnx,PDG19}.
  As such, the measurements of inclusive NC/CC DIS cross sections at the
  LHeC will have only a very moderate impact on the parameters of
  interest in a global EW fit, as it is briefly studied in
  appendix~\ref{app:globalfit}.
  Therefore, the strengths of an EW analysis of LHeC inclusive NC/CC DIS
  data are rather particular aspects of EW theory than actually the most
  precise values of the SM parameters itself.
  This will be studied in the following sections.
}

%--------------------------------------------------------
%  Higgs mass
%--------------------------------------------------------
One may also try to attempt to determine the Higgs-boson mass \mH\ from NC/CC
DIS data which enters through the self-energy corrections in the SM formalism.
However, the \mH\ dependence is only logarithmic,  
$\propto\log(\mH^2/\mw^2)$, i.e.\ very weak. A PDF+EW fit 
with the Higgs-boson mass as a free parameter leads an uncertainty
of $\Delta m_H=^{+29}_{-23}$ and $^{+24}_{-20}~\GeV$ 
for the LHeC scenarios LHeC-50a and LHeC-60b, respectively.
This compares well with 
the precision found for the indirect \mH\ determinations 
from LEP+SLC combined data~\cite{ALEPH:2005ab,deBlas:2016ojx,Haller:2018nnx}, but is, 
of course, much less precise than the direct determination 
from the LHC nowadays. 
...
%--------------------------------------------------------------------
\clearpage
\section{Prospects for the FCC-eh and EIC}
...
The EIC, HERA, LHeC and FCC are compared in
Fig.~\ref{fig:dSigmaFacilities}.
{\color{green}Two alternative versions of this plot are below}
%
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.46\textwidth]{plot_GraphsDsigmaDQ2_FCCEIC_opt}
...
\label{app:globalfit}
In a more general approach, the validity of the Standard Model is
frequently studied in so-called `global EW fits'~\cite{deBlas:2016ojx,Haller:2018nnx,PDG19}. 
In these statistical analyses a large number of precision
measurements are simultaneously fitted and compared to predictions in
a consistent theoretical framework.
The goals are on the one hand a comprehensive consistency test
of the SM formalism, and on the other hand the most precise and
comprehensive determination of (input) parameters of the SM.
%
It is of interest to estimate the potential input of LHeC inclusive
NC/CC DIS data, if those would become additional input to a global EW
fit.
This is studied in the following by performing a simplified global EW fit.
...
In a global EW fit a large number of precision measurements are
compared with optimised (fitted) theory parameters.
Commonly considered measurements are
$\alpha$ and \gf,
mass parameters \mz, \mw, \mH, and \mt, 
partial widths $\Gamma_W$, $\Gamma_Z$,
the effective weak mixing angle
$\sin^2\theta_\textrm {W,eff}^\ell$, %  $\sigma^0_\textrm{had}$,
partial decay widths and asymmetry observables in $e^+e^-$ collisions~\cite{ALEPH:2005ab}
$R_\ell^0$, $R_b^0$, $R_c^0$,
$A_\ell$, $A_c$, $A_b$,
$A_\textrm{FB}^{0,\ell}$, $A_\textrm{FB}^{0,c}$, $A_\textrm{FB}^{0,b}$,  
QCD contributions to the QED vacuum polarisation
$\Delta\alpha_\textrm{had}^5$, and sometimes also measurements of
 $\alpha_\textrm{s}$,  $m_b$, and $m_c$.
Under the assumption that $\alpha$ and \gf\ are known with ultimate
precision, the fit parameters chosen are:
\mZ, \mH, \mt, $\Delta\alpha_\textrm{had}^5$ and
$\alpha_\textrm{s}$; sometimes also $m_b$ and $m_c$.

For our purpose of estimating the impact of LHeC inclusive DIS
data as additional input to the global EW fit, and since we are
interested only in the uncertainties rather than
in the actual fit values, some simplifications are done for our following
`simplified global EW fit':
In a rational view, and since the fit is overconstraint, a global EW fit is a trade-off between the
experimental uncertainties of the measurements and the uncertainties
of their predictions that arise in turn also from the uncertainties of
the measurements (if theoretical uncertainties are ignored).
As such, it is observed that the measurement uncertainties of \mz,
\mH, $m_b$, 
$m_c$, $\Delta\alpha_\textrm{had}^5$ and $\alpha_\textrm{s}$ are much
smaller than any potential prediction. 
The uncertainties of the measurement of \mW\ and \mt\ and their
predictions are of somewhat similar size. All other observables have much
larger experimental uncertainties than their predictions.
Consequently, the constraints from a global EW fit
arises predominantly from the comparison of the measurement of \mW\ and
\mt\ with their predictions, which are obtained from the precision
measurements of $\alpha$, \gf, and \mz\ (with subleading contributions
from $\log\mH$, $m_b$, $m_c$, $\Delta\alpha_\textrm{had}^5$).

Therefore, we perform a simplified global EW fit, where we consider
(only) the measurements of \mZ, \mW, \mH\ and \mt. This is because all 
other parameters do not provide additional constraints or are known
sufficiently precise for their sub-leading relevance.
The calculations are performed in the on-mass shell scheme.
The fit parameters are chosen to be
\mW, \mH\ and \mt, while \mz\ is a
prediction\,\footnote{{\color{red}Note.} Even simpler, and equally
  good: Input measurements are \mZ, \mW, \mt, and fit parameters are \mW, \mt. }.
When using aboves measurements as input~\cite{PDG19} we obtain uncertainties of
$\Delta\mw=\pm5.6\,\MeV$ ($\pm6.2\,\MeV$) and $\Delta\mt=\pm0.87\,\GeV$
($\pm2.3\,\GeV$) when the respective measurements are included (excluded)
in the fit.
This is very similar to general global EW
fit~\cite{Haller:2018nnx,deBlas:2016ojx}, and confirms our
simplifications made, and is good enough for our purpose.
...
obtain a moderate improvement on \mW\ to $\Delta\mw=\pm5.5\,\MeV$.
...
compete with direct measurements of \mz\ or \mH, and the strong
correlation of \mW\ with \mt\ (c.f.\ fig.~\ref{fig:mWmt}) prevents a
relevant impact of LHeC data on these parameters in such fits.
...
\mt\ presented above, see Fig.~\ref{fig:mWmt}.}
{\color{red}
  It may be noteworthy, that this study represents the first global
  EW fit where simultaneously PDF parameters are determined.
}
{\color{brown}
...
$\sin^2\theta_\mathrm{W,\ell}^{\text{eff}}(\mz^2)$.
