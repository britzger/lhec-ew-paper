...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
After the discovery of the Standard Model (SM) Higgs boson at the CERN LHC
experiments, ATLAS and CMS~\cite{Chatrchyan:2012xdj,Aad:2012tfa},
the measurement of all fundamental parameters of the SM has
been completed with a remarkable precision.
To fully establish the validity of the theory of electroweak
interactions, explore the mechanism of electroweak symmetry breaking,
and study the mechanism of the generation of particle masses, further
high-precision electroweak measurements have to be performed.

Electroweak precision measurements are made at low energies, for instance
in neutrino-nucleon scattering~\cite{McFarland:1997wx,Zeller:2001hh}, polarised electron nucleon proton scattering~\cite{Androic:2018kni},
M{\o}ller
scattering or muon lifetime measurements~\cite{Tishchenko:2012ie},
%measurements of parity violation effects in atomic transitions,
%measurements of the electron magnetic moment,
and at the scale of the masses of the electroweak boson in
electron-positron~\cite{ALEPH:2005ab,Schael:2013ita} and
hadron-hadron colliders (see e.g.\ Ref.~\cite{Olive:2016xmw} and therein).


In deep-inelastic electron-proton scattering, electroweak
precision measurements have a long tradition with a number of
groundbreaking results~\cite{Prescott:1978tm,Prescott:1979dh,Ahmed:1994fa}.
At HERA, precision measurements had been limited by both the
$ep$ center-of-mass energy of $\sqrt{s}=920\,\GeV$ and the luminosities
of about $0.5\,{\rm fb}^{-1}$ recorded by the H1 or ZEUS
experiments~\cite{Aktas:2005iv,Abramowicz:2015mha,Abramowicz:2016ztw,Spiesberger:2018vki}. In contrast,
the FCC-eh with its
considerably increased center-of-mass
energy of $3.5\,\TeV$ and a targeted integrated luminosity of up to
$3\,{\rm ab}^{-1}$ will allow for unique precision electroweak
measurements in deep-inelastic scattering and perform tests
of new physics beyond the electroweak scale and
up to the TeV masses.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% intro
In the following, prospects for the determination of electroweak
parameters from inclusive NC and CC DIS data at FCC-eh are studied.
Additional direct measurements will provide further constraints, such
as measurements of Higgs boson (see
section~\ref{sec:FCCehHiggs})
and top-quark production cross sections (section~\ref{sec:FCCehTop}).
%
The uncertainty values of electroweak parameters are
estimated by performing fits of theoretical predictions to simulated inclusive NC
and CC DIS data (see sections~\ref{sec:FCCehIntro} and~\ref{sec:PDFs}
for details).
In order to account for correlations with uncertainties of the
PDFs, which are expected to be determined from the same data, the
fits include also the determination of parameters of the PDF
parametrizations.
The methodology follows closely the prescription outlined in
section~\ref{sec:FCCehPDFs} and Ref.~\cite{Spiesberger:2018vki}.
Electroweak effects are included in the calculations through 1-loop
weak
corrections~\cite{Bohm:1986na,Bohm:1987cg,Bardin:1988by,Bardin:1989vz,Hollik:1992bz},
and all calculations are performed in the on-shell
renormalisation scheme,
which utilizes the fine structure constant $\alpha$ and the weak boson
masses, \mZ\ and \mW, as main input parameters\footnote{
Note that in the on-shell renormalisation scheme the Fermi constant,
$\gf=\gf(\alpha,\mZ,\mW,\mt,\mH,...)$,
is a prediction~\cite{Sirlin:1980nh,Olive:2016xmw},
and the weak mixing angle is
defined at all orders by its relation to the weak boson masses, $\sw=1-\mW^2/\mZ^2$.}.
%to the electroweak corrections.
%Higher order %weak corrections further utilize the top-quark mass, \mtop,
%the Higgs-boson mass, \mH, and further fermion masses.
%Consequently, the Fermi constant is a prediction,
%$\gf=\gf(\alpha,\mZ,\mW,\mtop,\mH,...)$, and the weak mixing angle is
%defined to all orders as $\sw=1-\mW^2/\mZ^2$.
...
\begin{figure}[tbh]
\centering
%\includegraphics[width=0.75\textwidth]{\main/physics/img/figkappa}
\includegraphics[width=0.55\textwidth]{plot_GraphsDsigmaDQ2}
%\vspace{-4cm}
\caption{{Single differential cross sections for unpolarised $e^+p$ and $e^-p$ NC
    and CC DIS for FCC-ep in comparison to HERA measurements and LHeC
    expectations.
    The foreseen lepton beam polarisations of $\pm80\,\%$ will considerably change the
    expected NC cross section in regions, where $\gamma Z$ and pure $Z$
    exchange becomes important, whereas the CC cross section scales
    linearly with the lepton beam polarisation.
}}
\label{fig:dSigma}
\end{figure}
%
The neutral current (NC) and charge current (CC) DIS cross
sections for unpolarised inclusive DIS are displayed in
figure~\ref{fig:dSigma}.
While CC DIS is mediated exclusively by the $W$ boson, the
weak boson contribution to NC DIS becomes significant at higher scales,
$\Qsq\gtrsim100\,\GeVsq$, through $\gamma Z$ interference and pure $Z$ exchange~\cite{Klein:1983vs}.
...
\begin{figure}[tbhp]
\centering
\includegraphics[width=0.47\textwidth]{plot_couplings_u}
\hfill
\includegraphics[width=0.47\textwidth]{plot_couplings_d}
\caption{
  The expected uncertainties for the light-quark weak neutral
  couplings ($g_{Au}$, $g_{Ad}$, $g_{Vu}$, $g_{Vd}$)
  from FCC-eh inclusive DIS data
  compared with the Standard Model expectations and
  the currently most precise measurements.
  For comparison, also the prospects for the LHeC are displayed.
}
\label{fig:couplings}
\end{figure}
%
The determination of the vector- and axial-vector weak neutral
current couplings, ideally performed separately for any of the
individual quark or lepton flavor, represents a
major precision test of the electroweak theory.
Their precision measurements can provide constraints on various
BSM models.
The sensitivity to the light-quark vector and axial-vector couplings
to the $Z$ boson ($g_{Au}$, $g_{Ad}$, $g_{Vu}$, $g_{Vd}$)
is investigated by performing a $g_{Au}$+$g_{Ad}$+$g_{Vu}$+$g_{Vd}$+PDF
fit to the simulated NC and CC DIS data at the FCC-eh.
The two-dimensional uncertainty contours ($\Delta\chi^2=2.3$) are
displayed in figure~\ref{fig:couplings} for each single quark flavor and
compared to recent
measurements~\cite{ALEPH:2005ab,Abazov:2011ws,Abramowicz:2016ztw,Spiesberger:2018vki}.
While the current determinations from $e^+e^-$, $ep$ or $p\bar{p}$
data have all similar uncertainties, the FCC-eh will provide a
measurement with unprecedented precision.
Uncertainties smaller than about 1\,\% are expected.
Due to the $\gamma Z$ interference term, DIS data provide also  access to the sign of
the coupling parameters, and thus FCC-eh does not exhibit any
ambiguity as it is the case for the $Z$-pole measurements.


Also electron coupling parameters are accessible at the FCC-eh
and a determination of heavy-quark couplings can be explored
if additional
flavor-dependent final states are observed, e.g.\ in charm-production
or $b$-quark production.
Even if these parameters will be measured only with uncertainties similar to those of the high-precision
measurements at the $Z$-pole, FCC-eh will allow for a unique
measurement of the scale-dependence of the weak neutral current couplings in a single experiment.
For instance a considerable precision for \Qsq\ dependent measurements
of the light-quark weak neutral-current couplings
can be expected in the range of $\sqrt{\Qsq}$ from a few 10\,\GeV up
to about 1.5\,\TeV\ with uncertainties down to a few permille.
At low scales such measurements are limited by the dominating contribution of
the pure photon exchange, and at high scales by the kinematic limit
and the integrated luminosity.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% weak mixing angle
More explicitly, the effective weak neutral current couplings can be
expressed in terms of the $\rho$ and $\kappa$ parameters:
\begin{align}
  g_{Af} &= \sqrt{\rho_f}I_{L,f}^3 \, , \\
  g_{Vf} &= \sqrt{\rho_f}\left( I_{L,f}^3 -2Q_f\kappa_{f}\sw\right)
  \, .
\end{align}
$\rho$ and $\kappa$ receive \Qsq\ dependent higher-order
radiative corrections~\cite{Spiesberger:1993jg}.
The determination of the vector and axial-vector couplings can be
translated into a determination of the effective weak mixing
angle, $\sin^2\theta_w^{\rm eff}=\kappa_{f}\sw$.
For $\sin^2\theta_w^{\rm eff}$,
an ultimate precision of down to 1.0~permille can be achieved when using
FCC-eh inclusive DIS data.
More meaningful flavor specific measurements and measurements
in conjunction with the $\rho$ parameters, yield more conservative
estimates.
By exploiting the
\Qsq\ dependence of the inclusive DIS data, a unique scale-dependent
measurement of the effective weak mixing angle is feasible over two
orders of magnitude in $\sqrt{\Qsq}$.
Using cross section ratios, such as the polarisation asymmetry,
charge asymmetry, or the CC/NC cross section ratio, further improvements can be achieved
since systematic uncertainties are expected to cancel, at least partially~\cite{Blumlein:1987fd,AbelleiraFernandez:2012cc}.
On the theory side, a cancellation of the $\rho$ parameter in
ratios of cross sections, as it is the case
for $Z$-pole measurements or in polarised M{\o}ller scattering,
does not take place since pure photon exchange the
$\gamma Z$-interference parts, which do not carry the same
$\rho$ dependence, are not negligible.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% charged currents
Direct measurements of CC cross sections at the $W$-pole
are exclusively performed at hadron colliders; lepton colliders
would need a well defined initial-state neutrino beam.
This restricts the current availability of precise
measurements of charged current coupling parameters.
In DIS at FCC-eh, CC DIS cross sections can be measured with high precision.
From the theoretical point of view, this is possible because CC
scattering is mediated exclusively by $W$ bosons, i.e.\ no interference effects are present.
From the experimental side, precision measurements can be
performed since the event
kinematics can be fully reconstructed by recording the hadronic
final state.
While at HERA, the uncertainties of charged current cross sections have been
limited by the measurement of the hadronic final state, limited integrated
luminosities, challenging trigger requirements and the
moderate kinematic reach, FCC-eh will greatly improve on any of these
aspects.


The uncertainty of a $W$-boson mass determination is determined in a
\mW+PDF-fit (the $Z$-boson mass \mZ\ is an external input in this study).
The expected uncertainties of \mW\ are
\begin{equation}
  \Delta\mW = \pm9_{\rm exp}\pm 4_{\rm PDF}\,\MeV,
\end{equation}
where the dominating sensitivity to \mW\ arises from the
normalization of the CC cross section and the $W$-boson
propagator term $\tfrac{\mW^2}{\mW^2+\Qsq}$.
The expected precision of \mW, $\Delta\mW=10\,\MeV$, exceeds the
precision of the current world average~\cite{Tanabashi2018} ($\Delta\mW=\pm12\,\MeV$)
and is more precise by about a factor of two than the most precise
result of a single experiment~\cite{Aaltonen:2013vwa,Aaboud:2017svj} to date.
%
Current determinations of \mW\ from hadron-collider experiments
have a considerable PDF uncertainty of about $\gtrsim7\,\MeV$~\cite{Tanabashi2018}, but measurements at the FCC-eh
will provide further support for such precision measurements in
hadron-hadron collisions by its ability to perform combined
fits of electroweak parameters and PDFs.


%
%The expected uncertainty of \mZ\ of about 19\,\MeV\ can not compete with the precise
%measurements at the $Z$-pole, but still provides a complementary
%measurement of \mZ\ with remarkable precision.

In addition, the measurement of CC DIS cross sections allows for a
precise and unique test of the scale dependence of weak
charged current coupling parameters.
For instance, FCC-eh will allow for a test of the \Qsq\ dependence of
the CC form factor $\rho_{CC}$~\cite{Bohm:1987cg,Bardin:1989vz}.
The absolute value of $\rho_{CC}$ can be measured at FCC-eh with a
precision of down to 1.5~permille, and its scale dependence will be
measurable up to $\sqrt{\Qsq}\simeq1.5\,\TeV$ with an uncertainty of
about 2.2~permille.
At low scales, the FCC-eh measurement of CC DIS cross sections will be
limited by trigger constraints and at high scales by the available
integrated luminosity.

The measurement of charm production cross sections in CC DIS will be
possible for the first time with high precision.
...
FCC-eh provide a unique opportunity to perform high-precision
...
Outstanding measurements will be made for weak charged currents and
